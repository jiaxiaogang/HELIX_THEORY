...
* 起因：在38021中也边训练训调参了这五个竞争因子权重，不过发现无论怎么调，有的还是不够强，有的还是不够弱，所以本表，加强强的强度，降低弱的弱度，分别尝试制定更合理的策略机制。
* 说明：现在ST一共有五个竞争因子，以下分别按高中低排其重要性，并根据重要性制定竞争淘汰机制。
* cOuterShapeWeight // 高：头部保留（大于80%匹配保留）。
* cInnerEigenWeight // 高：头部保留（大于80%匹配保留）。
* cTotalCountWeight // 中：越高越好（中间部分自由竞争）。
* cBestsCountWeight // 中：越高越好（中间部分自由竞争）。
* cAverStrongWeight // 低：末尾淘汰（小于20%强度淘汰）。
* 总结：根据以上分析：
  - 1、外形内征必须达标80%（原因：因为不达标的它完全就不是这个东西）。
  - 2、匹配数总数进行自由竞争（原因：匹配范围越大肯定越好，不然只是局部匹配）。
  - 3、稳定性进行末尾淘汰（原因：太新的不稳定的，不能抢占资源，有了基础成长之后才有资格激活）。
