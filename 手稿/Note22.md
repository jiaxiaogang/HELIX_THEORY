# 六测(VRS多向飞行) & R-模式迭代 & 七八测(防撞训练) & 规划决策(子任务&ARSTime评价)
...
- [六测(VRS多向飞行) & R-模式迭代 & 七八测(防撞训练) & 规划决策(子任务&ARSTime评价)](#六测vrs多向飞行--r-模式迭代--七八测防撞训练--规划决策子任务arstime评价)
  ...
  - [n22p13 时间规划2-嵌套子任务(子需求)](#n22p13-时间规划2-嵌套子任务子需求)
  ...
  - [n22p20 八测3-觅食防撞融合训练](#n22p20-八测3-觅食防撞融合训练)
  - [n22p21 GL嵌套迭代](#n22p21-gl嵌套迭代)
  - [TODOLIST](#todolist)
...
| 22014 | 空S评价防失误杀迭代 |
| --- | --- |
| 问题 | 一棍子打死,终身牛逼,失误一次,打入地狱,太误杀; |
| 方案 | 根据可行率做ports排名,即乔丹失误一次还是牛逼,除非一直失误; |
...
| 22155 | 静默成功废弃后-等待木棒ActYes也被废弃的问题 |
| --- | --- |
...
从而导致R-模式也混乱,不够理性不够准确,本节将对时序多预测的准确性进行改进,其中`静默成功`将被废弃(参考22154),本节通过更准确的时序预测,来替代之;
...
| --- | --- |
| 示图 | ![](assets/432_子任务来的及评价网络结构示图.png) |
...
| 说明 | 已错过即将解决方案推进到已发生处 (即推进到已出门,看还有什么办法没); |
...
| 22201 | 觅食训练最终步骤 (参考22035) |
| --- | --- |
| 1 | `直投,右下飞,直投`, `边直投边飞至右上`x N |
| 2 | `重启,右投,飞至坚果,马上饿`x3 `左投,飞至坚果,马上饿`x3 |
| 3 | `重启,右投,马上饿` (会原地空吃,发现不行再飞至坚果吃) `左投,马上饿`; |

| 22202 | 防撞训练步骤 (参考22161) |
| --- | --- |
| 1 | `直扔木棒`x8向=>习得`F18[A8(无距棒)]->{mv-}` |
| 2 | `偏扔木棒,边右飞边扔`xN=>习得不被撞的经验`Fx->{mv0}` |
...
| 22203 | 无GL距小经验的BUG |
| --- | --- |
| 示图 | ![](assets/437_八测3无GL距小经验的BUG.png) |
| 说明 | 22201训练完,训练多次飞近且成功觅食过,但再后来就不成功,且没GL经验了; |
| 分析 | GL经验是存在的,只是未顺利取得; |
| 方案 | 将GL改成嵌套ports方式,使GL经验取用时更精准 (转至22204); |

| 22204 | GL经验不嵌套导致误杀的BUG |
| --- | --- |
| 示图 | ![](assets/438_GL经验不嵌套导致误杀的BUG.png) |
| 分析 | 两个问题:`1.GL经验被误杀`,`2.左飞经验有但未顺利取得`; |
| 方案 | 把GL改成嵌套ports方式,一来避免误杀,二来取用时更精准; |
| 分析 | 一: 上图中,未出现误杀,那些经验本来就无法稳定的飞近; |
|  | 二: 目前的absPorts+type方式一样可以精准取用,所以先不改嵌套方式; |
| 查代码 | 1. 查下getInner()方法如何取用GL经验 `转至22211`; |
|  | 2. 查下GL类比,的assFo联想方式是什么 `转至22212`; |
|  | 3. 查空S评价挂载在哪个fo下,会不会导致混用混乱 `T`; |
|  | > 空S评价会把`抽具象路径上`不稳定的挂上否,而稳定的不会,所以不会混乱; |
...
22203和22204两个BUG,说明GL经验的联想并不稳定,所以本节迭代getInnerV3(),使之有更稳定的GL联想结果;

| 22211 | getInnerV3迭代 `T` |
| --- | --- |
| 现网络 | ![](assets/438_GL当前网络结构图_参考21115.png) |
| 旧联想 | 当前联想路径:glValue->glAlg->backConAlg->glFo,然后与参考alg判断mIsC有关即可; |
| 分析 | 22203&22204的不稳定性,就是旧方式不够理性,所以应将联想路径反过来; |
| 方案 | 从fo出发,向自身+抽象,找glFo,然后判断与backConAlg有引用关系即可; |

| 22212 | GL内中外类比迭代 `T` |
| --- | --- |
| 加强抽象 | 改为类似dsPorts方式进行assFo联想,以加强抽象,使之迁移性更强; |

| 22213 | getInnerV3兼容HN分析 |
| --- | --- |
| 起点 | 在GL和HN时,传入的maskFo参数,情况是不同的; |
|  | 1. GL时,传入瞬时中匹配MC的matchAlg,所在的protoFo (即: 符合现实世界的当前场景); |
|  | 2. HN时,传入决策短时记忆中的alg.baseFo (即: 即当前解决方案matchFo); |
| 方向 | 分析二者在决策思维操作上的不同; |
|  | 1. GL时: `以往是怎么加工V的?` -> 向抽象取嵌套 |
|  | > 如: 这种现实情况,我们一般都直接躲开; |
|  | 2. HN时: `每次得到的坚果,是哪来的?` -> 向具象取嵌套 |
|  | > 如: 我饿的时候,都从小卖部买东西,或者抢小红的可乐喝; |
| 方案 | 即,GL与HN最大的区别在于,maskFo向着抽具象哪个方向进行联想找经验; |
| 结果 | 实而虚之,虚而实之,GL从于实而向虚,HN从于虚而向实; |
