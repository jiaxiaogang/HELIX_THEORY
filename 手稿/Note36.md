# GT识别：不准确问题 & 错位问题 & GT自举迭代

***

<!-- TOC -->

- [GT识别：不准确问题 & 错位问题 & GT自举迭代](#gt识别不准确问题--错位问题--gt自举迭代)
  ...
  - [n36p03 提升识别准确度、深入调试ST竞争浮现的细节](#n36p03-提升识别准确度深入调试st竞争浮现的细节)
  - [n36p04 提升识别准确度、深入调试GT竞争浮现的细节](#n36p04-提升识别准确度深入调试gt竞争浮现的细节)
  - [n36p05 提升识别准确度、回测GT竞争浮现（一）](#n36p05-提升识别准确度回测gt竞争浮现一)
  - [n36p06 提升识别准确度、回测GT竞争浮现（二）](#n36p06-提升识别准确度回测gt竞争浮现二)
  - [n36p07 GT自举GV实现（一）：解决ST错位问题](#n36p07-gt自举gv实现一解决st错位问题)
  - [n36p07B GT自举GV实现（二）：思考其后续SV实现](#n36p07b-gt自举gv实现二思考其后续sv实现)
  - [n36p08 GT自举GV实现（三）：回测GT竞争浮现](#n36p08-gt自举gv实现三回测gt竞争浮现)
  - [n36p09 GT自举GV实现（四）：迭代GT类比部分](#n36p09-gt自举gv实现四迭代gt类比部分)
  - [n36p10 GT自举GV实现（五）：回测GT竞争浮现](#n36p10-gt自举gv实现五回测gt竞争浮现)
  - [n36p11 GT自举切图实现（一）：解决GV错位问题](#n36p11-gt自举切图实现一解决gv错位问题)
  - [n36p12 GT自举切图实现（二）：测试](#n36p12-gt自举切图实现二测试)
  - [n36p13 提升ST识别准确度](#n36p13-提升st识别准确度)
  - [n36p14 提升识别准确度、回测GT竞争浮现（三）](#n36p14-提升识别准确度回测gt竞争浮现三)
...
**小结：36071从问题切入来直接分析问题，36072回到理论层面来深入分析问题。**
**其实：二者殊途同归，都是需要把GT自举算法减少一个维度。**
...
| 36102 | GT类比不出显著特征问题 |
| --- | --- |
| 示图 | ![](assets/795_类比到的absGT不显著问题.png) |
...
> 但是：assGT并非直接平辅在ProtoImg上，所以中途的那些best的Rect还是管用的，要以此影响到切图Rect。
```
...
| --- | --- |
| 示图 | ![](assets/798_0识别成1匹配率还高的问题.png) |
...
|  | 示图：![](assets/800_1识别成1的问题方案v2效果.png) |
...
| 示图 | ![](assets/801_识别对和错的结果匹配率一样问题.png) |
...
| 实践 | 实践如下，边实践方案2，边**主要进行特征识别的配置调整（边测试边训练边调整）。** |
...
| 小结 | 上面主要实践方案2和边测边调整了各个竞争配置，但效果仍不够好，下面继续。 |
| TODO5 | ... |
