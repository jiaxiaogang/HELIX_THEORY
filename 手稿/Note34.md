# 测试推进细节修正的问题，并整理推进 & 多码特征、固定粒度视觉支持
...
- [测试推进细节修正的问题，并整理推进 & 多码特征、固定粒度视觉支持](#测试推进细节修正的问题并整理推进--多码特征固定粒度视觉支持)
  ...
  - [n34p13 局部特征向似层找整体特征](#n34p13-局部特征向似层找整体特征)
  - [n34p14 多码特征回测E：测局部特征向似层找整体特征的识别与类比](#n34p14-多码特征回测e测局部特征向似层找整体特征的识别与类比)
    - [34142-继续慢测，手动触发几张鼠标照片，观察日志和代码跑的情况，关注下鼠标特征识别和类比的情况。](#34142-继续慢测手动触发几张鼠标照片观察日志和代码跑的情况关注下鼠标特征识别和类比的情况)
  - [n34p15 多码特征工作流程总结分享](#n34p15-多码特征工作流程总结分享)
  - [n34p16 组码索引性能优化](#n34p16-组码索引性能优化)
  - [n34p17 多码特征回测F：训练测试多种照片认知识别](#n34p17-多码特征回测f训练测试多种照片认知识别)
  - [n34p17B 多码特征回测F：训练测试遮挡、噪点、不全 的识别](#n34p17b-多码特征回测f训练测试遮挡噪点不全-的识别)
  - [n34p17C 特征训练计划：更广泛 与 更细节](#n34p17c-特征训练计划更广泛-与-更细节)
  - [n34p17D 认知特征细节之：显著度](#n34p17d-认知特征细节之显著度)
...
本节第一版方向是做组特征，第二版转向做特征嵌套，第三版又转向“局部特征向似层找整体特征”。
...
| 方案 | 可以增加组特征，每个抽象（局部）特征之间判断位置符合度，符合度高的列为了组，做为一个组特征。 |
...
| 34133 | 特征嵌套：方案3-细节分析与工程实践 |
| --- | --- |
| TODO1 | 组特征的表征不再是level,x,y，而是xywh `T`。 |
| TODO1.2 | 单特征也可以改成xywh `T`。 |
| TODO2 | 位置符合度算法迭代下v2，支持用xywh来计算 `T`。 |
| TODO3 | 因为组特征的嵌套，需要跟着迭代下特征识别算法，递归流程如下： |
|  | 1、局部T识别为assTs结果。 |
|  | 2、assTs结果打包成protoGT。 |
|  | 3、对protoGT进行特征识别（根据assT分别refPort来取以前构建的assGT组特征）。 |
|  | 4、根据位置符合度v2算法 和 复用匹配度等，识别到准确的assGT结果。 |
|  | 5、直到达到退出递归的条件，退出递归 `转TODO4`。 |
| TODO4 | 特征嵌套的递归退出条件：要看构建组特征到哪个宏观程度就中止？ |
|  | 分析：这个退出条件很难分析，怎么叫足够宏观的程度呢？ |
| 结果 | 此方案总感觉比较复杂，退出递归的条件不明，且识别时先识别T后打包GT也不太对，识别时原则上是不允许再构建新节点的 `转下表继续分析`。 |

| 34134 | 局部特征不准确问题之：方案4 |
| --- | --- |
| 说明 | 感觉方案3：V-GV-T...-A-F，这个越写越感觉不太对。 |
| 思路1 | 一来本来**局部特征其实就是抽象特征**，何必非要设想成微观层？ |
| 思路2 | 二来与局部特征对应的**整体特征GT不就是具象特征**吗？ |
| 思路3 | 那么我们何必舍近求远，直接还在原来抽具象关系上下功夫，就能解释当前过度局部导致识别不准确的问题吧？ |
| 方案4 | 特征识别assTs后，再向conPorts取具象整体特征（并判断位置符合度，抽具象局部特征匹配度），得出结果：做为更准确的特征识别结果。 |
|  | 优点1：性能好：即不构建protoGT，直接根据assTs分别取具象conPorts。 |
|  | 优点2：简单：不多构建GT，还是原来的T一层，也省得各种数据处理。 |
|  | 优点3：具象的特征本来就是整体的（具象即整体）。 |
|  | 优点4：正好具象的整体性，也可以做退出递归的条件。 |
|  | 优点5：正好相当向向具象找似层了（本来现在的识别算法就偏向似层结果）。 |
| 结果 | 如上，方案4一剑多雕，强烈建议采纳 `95% T`。 |

| 34135 | 局部特征与整体特征问题之：方案4-实践 |
| --- | --- |
| TODO1 | 在特征的抽具象port中存上xywh，特征整体性识别时要用 `T`。 |
| TODO2 | 特征识别算法在原识别基础上加step2：在识别到局部特征assT后，向具象开始找整体特征，在step2中conPort又是其新的assT `T`。 |
| TODO3 | 求整体特征的综合位置符合度 `转下表 34136 T`。 |
| TODO4 | 根据位置符合度排序，过滤掉符合度低的xx% `T`。 |
| TODO5 | 把旧特征识别重命名为step1（结果中似层部分可直接做为最终特征识别结果输出）`T`。 |
| TODO6 | step1的结果传给新的特征识别step2，向conPorts继续找似层结果（只保留似层结果） `T`。 |
| TODO7 | 最终只把似层结果输出，并且进行类比等，所以特征只有似交两层（只识别似层进行类比，所以交层最多只能上一层）`T`。 |
| TODO8 | step2的结果，是通过absT来计算assT与protoT的匹配度等的，但最终输出时，却需要assT直接与protoT的映射等数据 `转34137 T`。 |

| 34136 | 局部与整体特征的位置符合度计算 |
| --- | --- |
| 示图 | ![](assets/743_局部到整体特征位置相符度算法分析.png)![](assets/744_局部到整体特征位置相符度算法分析.png) |
| 说明 | 如上图，根据与proto的比例scale、方向direction、距离distance分别求相近度，然后乘积匹配度，具体计算依据如下方案： |
| 问题1 | 三者的计算都必须先缩放平移到与proto对齐才好对比：比例、方向和距离都必须考虑综合缩放多少，综合偏移多少的问题。 |
|  | 解答：即必须预估下：所有局部特征与assT对比时，对其统一缩放多少，偏移多少，所以如下方案1。 |
| 问题2 | 缩放平移对齐后，方向就不准确了，毕竟对齐后，局部相当于是抖动未完全对齐，用方向肯定是没规律的。 |
|  | 解答：还是改回用deltaX,deltaY吧。 |
| 方案1 | 用平均值当预估缩放偏移值：先排除首和尾各10%后，然后剩下的求平均值，以此来实现assT和protoT的对齐。 |
| 方案2 | 用扎堆为标准，每个assT对应的absT可能分成几组，这几组分别各自计算其缩放和偏移值。 |
| 抉择 | 方案1简单直白，方案2有点复杂但也似乎有可取之处，比如一眼就能看出一张图上多个手写0。 |
|  | PK1. 但方案2未必是优解，可能一眼识别多个0并不是一眼，而是持续观察来的。 |
|  | PK2. 并且第一版肯定优先选择简单直白的方案1。 |
|  | PK3. 并且同一个整体特征assT也不建议识别多个手写0，这也是有错误风险的。 |
|  | 结果：经过以上三次PK分析，暂选定方案1，实践如下： |
| TODO1 | 先掐头去尾，计算平均缩放比例，做为统一缩放比例 `T`。 |
| TODO2 | 统一缩放后，再掐头去尾，把平均偏移量算出来，做为统一偏移对齐数 `T`。 |
| TODO3 | 把整体特征assT的每个局部absT，都进行先按TODO1的结果缩放，再按TODO2的结果偏移，以实现与protoT对齐 `T`。 |
| TODO4 | 以proto为准，分别计算对齐后的scala相近度，deltaX相近度，deltaY相近度（离proto最近的为1，最远的为0） `T`。 |
| TODO5 | 将三个相近度乘积，求出当前assT的当前局部特征absT的 综合位置符合度 `T`。 |
| TODO6 | 整个assT的位置符合度 = 所有局部特征absT的位置符合度 的平均值 `T`。 |

```java
34136-前期草稿：是34136方案1和方案2的前期草稿，应该没什么用了。
/**
 *  MARK:--------------------用于对比absT在protoT的rect 和 absT在assT的rect = 两个rect的位置符合度--------------------
 *  @desc 方案1草稿说明：采取比例+偏移方案来对比位置符合度，即：
 ...
 * @desc 方案2草稿说明：采取比例+偏移方案来对比位置符合度，即：
 *          1、比例：先将两个rect都缩放成absAtProtoRect的大小。
 *          2、偏移：再求出deltaX和deltaY（因为先缩放成一样大小了，所以所有偏移都是平行的）。
...
| 结果 | 根据以上图，从这些absT中，取得整体absT所需的数据（详情见analogyFeature4Step2代码）。 |

**至此：`局部特征向似层找整体特征`的step2特征识别算法 和 step2特征类比算法 都开发完毕，下节测试。**

***

## n34p14 多码特征回测E：测局部特征向似层找整体特征的识别与类比
`CreateTime 2025.04.13`

```java
34101-先慢测，手动触发几个手写数字识别，观察日志和代码跑的情况。
* 测试项: 如下日志：由两个局部特征拼成整体特征，类似于把手写0的关键特征扣出来了。日志如下：
整体抽象特征T822（局部特征数:2 GV数:15）：{0_8 = 3;0_4 = 1;0_18 = 2;}
...
#### 34142-继续慢测，手动触发几张鼠标照片，观察日志和代码跑的情况，关注下鼠标特征识别和类比的情况。

* **步骤1-训练：最近鼠标识别的bug修个差不多了，跑了12张鼠标照片，12张手写0，如下：**

  1. 训练0用的这些图：  
  ![](assets/748_鼠标识别训练结果图1.png)

  2. 训练鼠标用的这些图：  
  ![](assets/749_鼠标识别训练结果图2.png)

* **步骤2-测试：训练完后，我测试一张新的鼠标照片看下识别效果，如下：**

  3. 测试新的鼠标图：  
  ![](assets/750_鼠标识别训练结果图3.png)

  4. 最终识别结果为65%为鼠标。  
  ![](assets/751_鼠标识别训练结果图4.png)

* **步骤3-总结：**

  5. 其实关于特征提取，验证，识别，这些过程的参数我还没调好，另外性能优化也还没做好，另外我用的数据量比较少（我的意思是：HE在做鼠标和0训练前，它是完完全全的婴儿系统，别说见过圆形的东西，它任何东西都没见过）。
  6. 当然我会继续调整参数（主要是关于竞争系数过滤强度等的调整）。

注意：鼠标和0的外形相似容易混，跑这俩能看它会不会搞混。然后最后用了新样式的鼠标来测试效果，也是看它判断像哪个。
...
4、从refPort.Rect到具象特征表征

![](assets/754_组码到特征.png)

二、特征识别方法A：根据组码 识别 局部特征  
说明：本方法顺着微观组码 的 refPorts关联，找出宏观特征。  
1、组码三索引的相近度识别（乘积匹配度）  
2、局部特征识别（位置符合度）  

![](assets/755_组码位置符合度.png)

3、识别结果的：匹配度竞争 & 符合度竞争。  
4、识别结果的关联强度+1 和 组码元素映射关系  
5、概念时序识别（此处不涉及不展开）  

三、特征类比方法X：具象特征中提取抽象特征  
1、识别匹配部分，做为类比依据，判断责任度过滤。  
...
四、特征识别方法B：借助局部特征 识别 整体特征  
说明：本方法顺着抽象特征 的 conPorts关联，找出具象特征。  
1、根据局部特征的conPorts，找到整体特征。  

![](assets/757_局部特征到整体特征.png)

2、根据计算：缩放+平移 实现对齐。  
3、根据局部特征在两个整体特征(protoT & assT)中的rect计算位置符合度。  
4、识别结果的：匹配度竞争 & 符合度竞争。  
5、识别结果的关联强度+1 和 局部特征映射关系。  

五、特征类比方法Y：借助局部特征 类比 整体特征  
1、根据局部特征的rect的匹配度和位置符合度结果进行类比。  
...
随着鼠标，水杯，雪球等图片训练，GV识别越来越慢，本节优化下。

| 34161 | 性能问题：GV识别时性能很慢。 |
| --- | --- |
| 性能 | 为什么GV那么大（占硬盘9x%都是GV占的），查下 |
| 疑点1 | GV的三个索引，是一维数组，这导致一个数组可能有几千条。 |
| 疑点2 | 经查每个GV在训练几十张图后，有20多KB，这显示太大了，读出来GV看下为什么那么大，都存啥了。 |
| 经查 | 疑点2没啥异常，就是这么大，疑点1有问题确实导致慢（GV识别循环太多，导致再简单的操作也慢）。 |
| 分析 | 现在的GV三个索引序列有问题，方案分析如下： |
| 方案1 | 把几千条由一维数组改成二维数组，以0.01精度的索引值为最大长度，分别指向每个值的GV数组 `5%`。 |
|  | 缺点：治标不治本，不会因为数组变成二维就节省循环数，节省性能。 |
| 方案2 | 代码里不要循环gvIndexes，直接对三个索引共三个gvIndexes，进行取交 `60%`。 |
|  | 优点：这一方案肯定有效，先取交能够解决几千条的性能问题。 |
|  | 缺点：九宫每格有100种，就是100的9次方，还有顺序，很容易达到巨量，随着GV越来越多，达到百万级后，读索引和取交都是奢侈的。 |
|  | 所以：我们就不该让GV达到百万种之多，方案如下： |
| 方案3 | 把9宫改成4宫 `5%`。 |
|  | 优点：这样数据量肯定会立杆见影的变少。 |
|  | 缺点：但感觉也是治标不治本，首先即使四宫最终GV数据量肯定上亿了，再次九宫变四宫它的解析性会变低，很可能辨别力下降，毕竟四宫像素排序太容易重复类似了。 |
|  | 线索：这里的关键问题在于，从三索引到九宫之间的映射，这俩是相乘关系，所以数据量很容易高。再改方案如下： |
| 方案4 | 直接以三索引做为GV.content，以减少GV的总量 `95%`。 |
|  | 优点：此方案比较彻底，直接把三索引和九宫合一了，不再存在相乘关系。 |
|  | 优点：最终GV的数据量最多也就100x100x100个，然后每个索引最多指向100x100个GV（即三个索引各自最多长度1w）。 |
|  | 缺点：GV内容更有损了，在还原原图时，会有些失真。 |
|  | 抉择：该方案缺点可接受，优点非常巨大，所以优先考虑。 |
| 结果 | 方案2和方案4可以一起执行，方案4为主，方案2为辅，实践如下：。 |
| TODO1 | 方案4-把GV构建时，改成先转成三索引值，然后将索引做为V进行装箱 `T`。 |
| TODO2 | 方案4-然后再将三索引值做为GV.content装GV的箱 `T`。 |
| TODO3 | 方案4-这样GV构建时也是根据三索引值来防重 `T`。 |
| TODO4 | 方案2-先收集最终全含有效，再进行相近度乘积竞争等操作（写个和特征识别类似的模型，先收集后竞争，最终输出识别结果）`不需要 T`。 |
|  | > 方案2不需要写模型，只要最内部for循环除了objectForKey和加减乘除外，不要有复杂的操作，它速度就不会慢 `T`。 |
| TODO5 | 把三个索引的解析度（值条数）由100个改成20-36个左右，这样GV总数变少，内存占用变小 `T`。 |
| 结果 | 方案4改完了，但经回测性能效果不好，细细想来，方案4调整成三个索引对应多个GV，与方案1又有什么区别呢？`转34163调试优化代码 T`。 |

| 34162 | 更多的可能的优化方案（以后再说） |
| --- | --- |
| 方案5 | 组码反编影响大时，可考虑加：组码第四索引，比如“曲度索引 或 什么的”。 |
| 方案6 | 或者考虑改回废弃组码，直接用单码hsb做索引？多粒度还要保留，这样的话，其实相当于是只保留组码的均值索引，另外两个不要了。 |
| 方案7 | 看要不要改成抽具象关联和引用加强最后再异步调用，识别时，仅仅取结果就行（或者只对这些代码加个异常执行即可）。 |
...
| 34165 | 继续手跑0，测匹配度符合度，能慢慢变高。 |
| --- | --- |
| 训练 | 手动跑到第20个手写0时，特征识别结果中GV数越来越少，此问题得查下。 |
| 思路 | 抽象越来越多时，GV就越来越少，此特征慢慢变的不再健全，只剩一个微小的信息点。 |
| 方案 | 所以加上健全度计算（健全度 = assCount/protoCount），并做为局部特征识别的第三个竞争因子。 |
| 修复 | 健全度：把局部特征识别算法中，竞争因子加一个“assCount/protoCount”到乘积中，这样就能越来越趋向于保留GV数大些的。 |
| 结果 | 修复后，经因测0的识别匹配度符合度都比原来可以越来越高了。 |
| 回测 | 训练20个0后，局部特征匹配度80%&符合度60%、整体特征匹配度50%&符合度90%、局部概念匹配度80%、整体概念匹配度55%。 |
| 快照 | 将此训练结果存为FZ1051，代码分支FZ105训练源分支 `T`。 |

***

## n34p17 多码特征回测F：训练测试多种照片认知识别
`CreateTime 2025.04.23`

```java
34171-多种照片识别：
## 简介：上节末用的是Mnist图里的手写0来训练的，本节，我将对自己从不同背景不同角度拍的“鼠标、杯子、手柄、猫咪、饮料”各5张照片来训练，观察它从没见过某物，到最后认识某物的过程。具体如下：
## 注意：我自己拍的图，处理成2000像素，并且角度不同背景不同，其实非常模糊，这些图我会放在FZ105快照中一份。
...
## 第2步、训练5张鼠标照片
* 日志：
 169 [22:15:03:535 TI           TIUtils.m 645] (鼠_1)概念识别结果总结：(0=1.00 )
1650 [22:15:15:379 TI           TIUtils.m 645] (鼠_2)概念识别结果总结：(0=0.80 ,鼠=0.20 )
3222 [22:15:28:135 TI           TIUtils.m 645] (鼠_3)概念识别结果总结：(鼠=0.67 ,0=0.33 )
4435 [22:15:29:135 TI           TIUtils.m 645] (鼠_3)概念识别结果总结：(鼠=0.86 ,0=0.14 )
6013 [22:15:37:791 TI           TIUtils.m 645] (鼠_4)概念识别结果总结：(鼠=1.00 )
8643 [22:15:49:136 TI           TIUtils.m 645] (鼠_5)概念识别结果总结：(鼠=1.00 )
* 说明：可以看到，第一次看到鼠标时，把鼠标认成“手写0”了，第2次识别成鼠标也像0，第3次开始就识别86%是鼠标了，第4次完全100%确认这就是鼠标，不是0。

## 第3步、训练5张杯子照片
* 日志：
  208 [22:19:49:078 TI           TIUtils.m 645] (杯_1)概念识别结果总结：(鼠=1.00 )
 3653 [22:19:58:993 TI           TIUtils.m 645] (杯_2)概念识别结果总结：(鼠=0.86 ,杯=0.14 )
 6481 [22:20:00:668 TI           TIUtils.m 645] (杯_2)概念识别结果总结：(鼠=0.55 ,杯=0.45 )
 8518 [22:20:06:806 TI           TIUtils.m 645] (杯_3)概念识别结果总结：(鼠=0.65 ,杯=0.35 )
11886 [22:20:13:718 TI           TIUtils.m 645] (杯_4)概念识别结果总结：(杯=0.61 ,鼠=0.39 )
14910 [22:20:20:503 TI           TIUtils.m 645] (杯_5)概念识别结果总结：(杯=0.62 ,鼠=0.38 )
18794 [22:20:23:382 TI           TIUtils.m 645] (杯_5)概念识别结果总结：(杯=0.74 ,鼠=0.26 )
* 说明：同样每张看一次并进行识别，刚开始把杯子认成鼠标了，后来慢慢认成杯子，不过杯子的图比较。。。可能它没法很快100%确认特征。

## 第4步、训练5张手柄照片
* 日志：
  222 [22:26:37:123 TI           TIUtils.m 645] (柄_1)概念识别结果总结：(鼠=0.65 ,杯=0.35 )
 3347 [22:26:42:508 TI           TIUtils.m 645] (柄_1)概念识别结果总结：(鼠=0.48 ,柄=0.28 ,杯=0.24 )
 6369 [22:26:51:641 TI           TIUtils.m 645] (柄_2)概念识别结果总结：(杯=0.62 ,鼠=0.19 ,柄=0.18 )
 9384 [22:26:55:054 TI           TIUtils.m 645] (柄_2)概念识别结果总结：(杯=0.45 ,柄=0.39 ,鼠=0.16 )
11884 [22:27:05:629 TI           TIUtils.m 645] (柄_3)概念识别结果总结：(柄=0.61 ,鼠=0.24 ,杯=0.15 )
14693 [22:27:07:819 TI           TIUtils.m 645] (柄_3)概念识别结果总结：(柄=0.62 ,鼠=0.23 ,杯=0.15 )
17278 [22:27:22:358 TI           TIUtils.m 645] (柄_4)概念识别结果总结：(柄=0.72 ,鼠=0.15 ,杯=0.13 )
20551 [22:27:25:475 TI           TIUtils.m 645] (柄_4)概念识别结果总结：(柄=0.76 ,杯=0.13 ,鼠=0.10 )
23663 [22:27:35:124 TI           TIUtils.m 645] (柄_5)概念识别结果总结：(柄=0.86 ,杯=0.13 ,鼠=0.01 )
27029 [22:27:39:574 TI           TIUtils.m 645] (柄_5)概念识别结果总结：(柄=0.86 ,杯=0.12 ,鼠=0.01 )
* 说明：不说明，与上面类似。

## 第5步、训练5张猫咪照片
* 日志：
  229 [22:29:57:709 TI           TIUtils.m 645] (球_1)概念识别结果总结：(柄=0.66 ,鼠=0.21 ,杯=0.13 )
 2576 [22:30:03:524 TI           TIUtils.m 645] (球_1)概念识别结果总结：(柄=0.39 ,球=0.37 ,鼠=0.14 ,杯=0.10 )
 4724 [22:31:50:981 TI           TIUtils.m 645] (球_2)概念识别结果总结：(柄=0.48 ,球=0.26 ,鼠=0.19 ,杯=0.07 )
 7412 [22:32:01:084 TI           TIUtils.m 645] (球_3)概念识别结果总结：(柄=0.45 ,球=0.28 ,鼠=0.25 ,杯=0.03 )
10054 [22:32:03:123 TI           TIUtils.m 645] (球_3)概念识别结果总结：(球=0.40 ,柄=0.40 ,鼠=0.16 ,杯=0.04 )
12113 [22:32:15:481 TI           TIUtils.m 645] (球_4)概念识别结果总结：(球=0.68 ,柄=0.20 ,鼠=0.12 )
14874 [22:32:24:310 TI           TIUtils.m 645] (球_5)概念识别结果总结：(球=0.52 ,柄=0.43 ,鼠=0.05 )
* 说明：不说明，与上面类似。

## 第6步、训练5张饮料照片
* 日志：
  226 [22:35:06:441 TI           TIUtils.m 645] (饮_1)概念识别结果总结：(球=0.60 ,柄=0.30 ,鼠=0.10 )
 3087 [22:35:36:634 TI           TIUtils.m 645] (饮_2)概念识别结果总结：(球=0.38 ,柄=0.30 ,杯=0.23 ,饮=0.10 )
 5933 [22:35:41:445 TI           TIUtils.m 645] (饮_2)概念识别结果总结：(球=0.45 ,饮=0.24 ,杯=0.18 ,柄=0.13 )
 8363 [22:35:53:465 TI           TIUtils.m 645] (饮_3)概念识别结果总结：(鼠=0.32 ,杯=0.25 ,柄=0.18 ,饮=0.16 ,球=0.09 )
10621 [22:36:05:312 TI           TIUtils.m 645] (饮_4)概念识别结果总结：(柄=0.38 ,球=0.32 ,饮=0.20 ,鼠=0.05 ,杯=0.05 )
13141 [22:36:15:920 TI           TIUtils.m 645] (饮_5)概念识别结果总结：(柄=0.32 ,饮=0.24 ,杯=0.19 ,球=0.16 ,鼠=0.09 )
15473 [22:36:20:941 TI           TIUtils.m 645] (饮_5)概念识别结果总结：(饮=0.29 ,杯=0.26 ,柄=0.24 ,球=0.16 ,鼠=0.06 )
* 说明：可以看到饮料并不那么准确，当然饮料的照片也多有手部遮挡等，但he4o的特征抽取应该不会受遮挡太大影响，明天查下为什么饮料识别率低。

## 结果：本表存为FZ1052，可见目前在感认知识别上，能够快速响应特征提取等，应该没什么大的BUG了（当然还得继续测测，肯定还有没发现的bug）。
## 另外：关于准确性，HE没有反向传播都是正向流程，此处日志还没到“概念的作用（是否有用）”那一块，到了准确率还会有一个大的提升。
```

***

## n34p17B 多码特征回测F：训练测试遮挡、噪点、不全 的识别
`CreateTime 2025.04.24`

| 34172 | 测遮挡、噪点、局部照片 等的测试 |
| --- | --- |
| 说明 | 用牙签一盒，一根，多根，互相交叉，各种背景，加噪点，局部照片等进行测试，最后识别结果还ok。 |
| 链接 | https://zhuanlan.zhihu.com/p/1898510362254500051 次日更新部分。 |

***

## n34p17C 特征训练计划：更广泛 与 更细节
`CreateTime 2025.04.24`

| 34173 | 下一步测试计划：主要面向更广 或 更细。 |
| --- | --- |
| 计划1 | 找ImageNet图库来跑下看。 |
| 计划2 | 认字，用正体字训练，用手写字测效果。 |
| 计划3 | 能不能识别字体（如宋，楷，黑） 或者 绘画风格（如油墨，素描，写实，写意）。 |
| 计划4 | 识别人脸（如动漫人物）。 |
| 计划5 | 识别背影。 |
| 结果 | 先写一个视觉训练工具，以应对更多图片的训练时方便一些 `T`。 |

| 34174 | 下一步：优先级 |
| --- | --- |
| 1、细节 | 先用人脸，或鸟品种，猫品种什么的。 |
|  | 重点：细节训练最重要的是找出显著的特征，即必须判断提取的所有抽象特征的显著程度，越显著的越是关键特征。 |
|  | 分析：`显著特征`可以提高识别率，看下把这个“显著度”计入特征识别step1算法竞争中（这是step1特征识别算法第四个竞争因子了）。 |
|  | 方案：用抽具象关联强度来实现，即“step1中assT被抽象的强度总和 除以 最大总和 得出显著度”，在特征step1识别中竞争时乘上它 `T`。 |
|  | 回测：经回测，加上显著度后，识别准确率下降明显 `第四竞争因子先去掉，然后转34175，继续改进看怎么做`。 |
| 2、广泛 | 用imageNet里找类别来跑。 |
| 3、感觉 | 可以优先用绘画风格来跑（比如油彩，素描等）。 |
...
通过下一步开发的几个需求，分析现在还有哪些地方需要改进。

| 34175 | 显著度公式及作用方法 |
| --- | --- |
| 需求1 | 34174-1回测：查下为什么加上显著度后，识别准确度下降（用多物DEMO训练过程可复现）。 |
|  | 问题：表面看sumConPortStrong总强度表达了“显著度”，实则不是，它与“健全度”相关于逆运算了。 |
|  | 即：显著度是越强越好（被抽象的越多），而健全度是越全越好（被抽象的越少），二者正好相抵。 |
| 一 | 思路1：二者矛盾，那是否可以分主次用相对来解决矛盾。 |
|  | 方案1：首先要保证有基础内容，即健全度为主，其次再保证健全后的显著度要高：即先根据健全度做末尾淘汰，再pk显著度。 |
|  | 缺点1：此方案并不是用“相对”做各自取优，更像是限制小范围的对抗，而sumConPortStrong会慢慢蚕食最终走向过度抽象的节点 `弃 T`。 |
|  | 公式1：经分析，每次抽具象关联时，会增强抽具象关联，同时会把每个refPort.strong+1，所以其实sumConStrong x assT.count就 = sumRefStrong。 |
| 二 | 分析2：虽然二者矛盾，但人家“健全度”没有问题，这里显然只是单方面的“显著度”的锅，所以还是直接解决此错误。 |
|  | 思路2：关键在于“sumConPortStrong”受到了场景外的影响，被外部影响的pk是没有独立人格的，所以应该把“显著度”限定在当前场景内。 |
|  | 方案2：显著度只根据当前assT的absPort.strong来，而不是根据absT.sumConPortStrong来。 |
|  | 示例2：A风希的头冠就是以往所有assT风希的显著特征，B而不是黑发是所有人的显著特征，所以A才是当前场景下的显著特征，而非B。 |
| 三 | 疑问3：step1识别到的就是局部特征，此时proto和assT没有抽具象关联，也没有单条conPort.strong可以取。 |
|  | 思路3：所以方案2还是不太行，在工程实践上不可行，此时压根没有单条抽具象强度来做显著度。 |
|  | 方案3：所以应该在特征step2识别算法中，才能找着具体的某一个抽具象关联场景，来做显著度。 |
|  | 示例3：在step2中识别成人类，那么头就是显著的。识别成风希，那头冠就是显著的。识别成眼睛，那眼光就是显著的。 |
|  | 细节3：将局部特征识别结果的itemAbsTs，在整体特征识别时指向assT的总强度，用于计算显著度。 |
|  | 公式3：显著度 = 当前itemAbsTs的总抽象强度和 / assT.absPortPorts总强度和 `T`。 |
| 结果 | 根据方案3，看来需要在特征step2识别算法中，根据itemAbsT和assT的关联强度，来计算显著度。 |
| TODO | 算出显著度后，做为整体特征识别的第三竞争因子 `T`。 |

| 34176 | 显著度可视化 |
| --- | --- |
| 需求2 | 需要做个`显著度`可视化工具，比如把显著特征的信息直接用高亮点阵方式显示在`视觉训练工具`的预览图上。 |
| TODO1 | 把step1Model的可视化改成一个itemAbsT一幅 `这个随着需要随手改就行 T`。 |
| TODO2 | 可视精度：把可视化支持每个GV解析出来三索引值，用于推断出9宫色值，然后可视化 `T`。 |
| TODO2.1 | 可视精度的子级优先级大于父级，所以先推断父级色值，再推断子级，如果父级有的色值子级又有，则用子级的 `T`。 |
| TODO3 | 可视色彩：把可视化支持每个HSB合一显示，即用HSB三个值，合成一个最终色值，然后再显示成可视化 `T`。 |

| 34177 | 提升特征识别结果竞争力度 |
| --- | --- |
| 需求3 | 现在`触发特征的类比太多了`，查下是不是识别结果保留的太多了，增加竞争，看保留更准确部分结果。 |

| 34178 | 分析HSB三通道能不能合一，以提升识别性能 |
| --- | --- |
| 说明 | 三个通道特征要识别T三次，性能因此不好，能不能把三个通道合一？比如65000色，就把这65000归1化，来表征？ |
| 分析1 | HSB的色相是循环值，所以最好改成RGB表征，再思考此需求的可行性。 |
| 分析2 | 匹配度不好算，比如：RGB1=(70,70,70) RGB2=(80,70,70) RGB3=(70,80,70)，那么R2和R3分别与R1的相近度是一样的，那我们如何区分？ |
| 分析3 | 即，最终肯定会表征成RGB三维数据，差值也会表征成三维数据，平均值也变成三维数据，V识别激活也由一维线性，变成三维区域激活。 |
| 问题 | 其实想想，如果这么搞，和现在的三个通道分别识别又有什么区别呢？ |
| 所以 | 按这个做法，其实和原来的第一通道ref结果，和第二通道ref结果取交，再和第三通道ref结果取交，是等效的。 |
| 结果 | 还是算了，现在的概念三特征全含，也是同理的，按本表这么改非要合一，也合不好到哪里去 `否掉 T`。 |

| 34179 | 分析是否有损过度或多粒度压缩过度 |
| --- | --- |
| 说明 | 主要分析在有损压缩中，是否有严重色块缺失或色值错误过大的问题。 |
| 分析 | 1、现在是当前粒度级差值90%相同，则由其父级来填充。 |
|  | 2、其父级GV可以根据三索引推断9宫色值，所以填充下来应该大差不差。 |
|  | 3、其子级GV也可以根据9宫来进一步进行填充，所以填充下来应该不会差哪去。 |
| 结果 | 现在的有损压缩方式未见明显异常，不必改动 `未发现问题 T`。 |
...
尾端备忘：
1. 待测1：`用时序SP稳定性测特征的合理抽象度`。
2. 待测2：`再回测简化H嵌套`。
2. 待测3：`测下特征类比还有没BUG：先用0跑跑看，能不能测得类比效果符合预期。如果不好观察效果，就搞几张专用简线图，用来测试类比效果，是否符合预期`。
