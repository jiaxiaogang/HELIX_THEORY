...
| 23077 | 偶尔距大于0通过VRS评价的问题 |
| --- | --- |
| 示图 | ![](assets/471_偶尔距大于0通过VRS评价的问题.png) |
...
| 23224 | Y距35含P评价的问题 (转自23223-BUG1) |
| --- | --- |
| 复现 | `FZ19,直击` |
...
| 方案 | 在PMV中,将理性和感性都已发生完毕的R任务销毁掉; |
| 代码1 | R新帧输入时,更新到cutIndex+1,将旧任务抵消掉 `RMV代码本就支持`; |
| 代码2 | P结果输入R的同区价值时,且cutIndex已全发生完,应销毁任务 `见方案`; |
| 代码3 | 当R任务因为发生完毕被销毁时,应该触发反省类比 `正向反馈类比本就支持`; |
| 结果 | 方案代码2改完后,BUG成功修复,可以尝试重新训练F19看是否还有Y距35的P; |
| 结果 | 此BUG修复前影响FZ19训练过程,按FZ19原步骤重训得FZ20 (参考23227); |

| 23225 | Y距35的S强度太弱的问题 (转自23223-BUG2) |
| --- | --- |
| 问题 | 主要是`强度一直是1,为什么`; |
| 结果 | 23224修复后,重新训练FZ20后,此BUG不复现,自然解决了; |

| 23226 | Y距的GL经验为0的问题 |
| --- | --- |
| 复现 | 在`FZ20,右上飞,直击`,可正常对Y距13PM评价为否,且转GL加工Y距107; |
| 问题 | ![](assets/524_Y距的GL经验为0条问题.png) |
| 方案 | 加训GL经验 (直击飞完前点下飞,飞完后木棒就不可见了) `参考23227`; |
|  | 问题:`下飞后看不到木棒` 修复:`取消视觉范围限制,改动画取坐标方法,后ok`; |
| 调整 | 环境调整: 可以调整让木棒可以停留下来,方便训练GL; |
| 训练 | 按照23227第5步训练->得到FZ21 (但回测GL仍是0条,转23228); |

| 23227 | 训练步骤整理 | 训练目标 |
| --- | --- | --- |
| FZ20-1 | 边直击边飞至左下 | 被撞和不被撞经验 |
| FZ20-2 | 边直击边飞至右上 | 被撞和不被撞经验 |
| FZ20-3 | 在安全地带各处偏击10次 (危险地带下方) | 加训P经验 |
| FZ20-4 | 在危险地带各处直击10次 | 加训S经验 |
| FZ21-5 | 直击,撞到前停住,飞:`↑↓→↓↗↑←↑` | 加训L经验 |
| FZ22-5 | 直击,撞到前停住,飞:`↓↘→↗↑↖←↙↓↘` | 加训GL经验 |

| 23228 | Y距的GL经验为0的问题2 |
| --- | --- |
| 问题 | ![](assets/526_Y距的GL经验为0条的问题.png) |
| 复现 | `FZ21,左上飞,直击`,看到getInnerGL()取到仍是0条; |
| 调试 | ![](assets/525_FZ21第5步训练GL经历分析.png) |
| Q1 | 经查FZ21第5步训练日志,GL经验经历的全是↑↓↗,没↖,所以是0条不稀奇; |
|  | 所以,本问题中,`FZ21,左上,直击`取到许多GL0条都是正常现象; |
| Q2 | 但也有抽象GL:F104,F98,而它们又挂在F368,F382下; |
|  | 所以,本问题示图中,从MaskFo:F368和F382取到0条是不正常的; |
|  | A2: 本来F104和F98都是L类型,而示图中找G是0条正常的; |
|  | 出路1: 当最近的G失败时,再尝试次近的G/L,这个需要迭代PM `转24011` |
|  | 出路2: 重训第5步(参考23227-FZ22-5),对Y距大也做GL抽象,得到FZ22; |
|  | 结果: `出路1复杂,转至24011`,`出路2已重训,且有效,见23229示图`; |
| 结果 | 如23229图示,在`Q2-出路2`重训FZ22后已解决了GL为0的问题; |

| 23229 | Y距的GL经验太抽象问题 |
| --- | --- |
| 示图 | ![](assets/527_23228回测ok_23229GL太抽象问题.png) |
| 复现 | `FZ22,左上,直击`; |
| 说明 | 虽然有了GL,但太过抽象不稳定,本表分析解决此问题; |
| 方案1 | 看能否将具象GL也挂在抽象时序(如F369)下 `5%`; |
|  | 此方案将抽具象放到同级竞争,但以往仅有过将过抽象的嵌在具象节点下 |
|  | 而非将过度具象的挂到抽象下,这会导致场景判断错乱; |
|  | 比如: 把给汽车打气嵌到车打气下,火车打气就会失败; |
| 方案2 | 或者顺着抽象GL向具象取方案 (即从F95,F99取具象GL经验) `5%`; |
|  | 此方案改动小,符合以往从上到下向性的做法,只需顺着具象找更稳定的即可; |
|  | 但从上到下会导致取到与当前场景不相符的GL经验,所以此方案pass; |
| 方案3 | getInnerV3_GL中,符合当前场景更全面的curMasks `待实测`; |
|  | 说明: 以往curMasks仅使用了absRFos不够全面,现扩展到matchRFos; |
|  | ![](assets/528_23229方案3中途问题.png) |
|  | 如上图,F103太具象,无法被识别为matchRFo,所以也没法从它找GL; |
|  | 解决: 重训FZ22,多经历一次左上GL训练,使之能抽象; |
|  | 跟进: 重训FZ22后也依然无效,因为很难抽象出[飞方向,木棒]的节点; |
|  | 1. 内中外类比的assFo是取absRFos与其具象得出 (具象混乱 & 抽象失真); |
|  | 2. 很难抽象出[飞方向,木棒],导致识别时也没法识别为[飞方向,木棒]; |
|  | 3. 进尔导致在getGL时,也没法找到GL经验; |
| 结果 | 方案3得出,GL的体与用协作出现了问题,转24012; |
