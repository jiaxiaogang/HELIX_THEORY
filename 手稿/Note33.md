...
- [(觅食 & 飞躲 & 踢搬运)三项的多向连续训练,以及三项融合训练](#觅食--飞躲--踢搬运三项的多向连续训练以及三项融合训练)
  ...
  - [n33p16B 回顾多向觅食训练之: 各方向学飞吃 & 多步飞吃](#n33p16b-回顾多向觅食训练之-各方向学飞吃--多步飞吃)
...
|  | > 当前读actionFos并不是所有的rCanset，它取H场景源（求解范围）并不全面。 |
| 正据5 | 重要：主要是能够解决H嵌套的过度复杂问题，可以简化代码，简化各种迁移关联，迁移，等的复杂度。 |
| 正据6 | 根据以上`正据`分析，感觉也只是把挂在RCanset下，改成挂在RScene下，别的不用动。 |
| 正据7 | 现在H之外还没再嵌套H，但其实它是应该支持的，如果H下再嵌套H，那么迁移将更加复杂，这种程度的复杂是万不可取的。 |
|  | > 结构说明：子解挂在子场景下并不只是多一层嵌套，因为子又有子它会变成多层嵌套。 |
|  | > 性能说明：直观看，这种多层嵌套是更准确的，但性能上压根不允许。越是把解藏在犄角旮旯，我们就越难再找到这个解。 |
...
| 总结 | 以上：`正据1和3和6的没有副作用`，以及`正据2的能从根本上解决继承滞后问题`，以及`正据4和7的性能考虑`，以及`正据5的大大简化数据结构和控制的复杂度`。 |
...
| TODO4 | **废弃I层Canset**，所有的canset都挂在F层RScene下，所有的OutSPDic都挂在这个F.canset下 `T`。 |
...
| TODO9 | 因为最近的：H简化层级，OutSPDic挂载变动，废弃iCanset等调整，把所有canset/scene/From/To等命名整理下 `T`。 |
...
| TODO2 | 父非子，根据迁移关联，把父SP的值，作用于子的评分 `T`。 |
...
**结果：最好别这么做，跨IF树很可能带来跨场景的不准确，因为SP稳定性是仅作用于当前场景的，不会因为Canset一样就能跨越。**
**所以：Canset全局防重并更远的同步OutSP值，一定要慎重做，要保证不能让它影响准确性的基础上，再去考虑这件事的可行性。（轻易别尝试）。**
