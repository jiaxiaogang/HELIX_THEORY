# 加权求和切图法 & 从绝对准度转向竞争浮现为准 & 规模化高清图视觉训练 & 视觉注意力递归实现 & 视觉注意力吸引机制
...
**二、关于视觉注意力机制目前总结了少数几种：**
1. 任务注意力：在工作记忆树上，有共同抽象的。
...
**暂未实践：**
1. n37p17 特征识别优化（四）：动体注意力
2. n37p20 特征识别优化（五）：无意注意力（不用实践，与任务注意力是流程一模一样的）。

**待回来继续：**
1. 等n38p01中的训练ok后，再回来继续做37192-摄像头视觉和注意力等视觉循环。
2. 关于`视觉注意力递归`，到Note38时，会以视觉与内核接洽的方式来实践。

***

<!-- TOC -->

- [加权求和切图法 \& 从绝对准度转向竞争浮现为准 \& 规模化高清图视觉训练 \& 视觉注意力递归实现 \& 视觉注意力吸引机制](#加权求和切图法--从绝对准度转向竞争浮现为准--规模化高清图视觉训练--视觉注意力递归实现--视觉注意力吸引机制)
  ...
  - [n37p15 视觉注意力递归：从良性循环到任务注意力](#n37p15-视觉注意力递归从良性循环到任务注意力)
  ...
  - [n37p19 视觉注意力递归：任务注意力实践](#n37p19-视觉注意力递归任务注意力实践)
  - [n37p20 特征识别优化（五）：无意注意力](#n37p20-特征识别优化五无意注意力)
...
方案3、100%：完全使用上面的流程来，整个思维都参与进来。
  缺点：方案3虽然正确，但测试范围太广了，现在HE也没有那么丰富的需求（如无意注意力等都没有），要向饥饿和疼痛两个Demand靠，也不容易，毕竟现在的坚果Demo和Minst图的Demo没打通，挺麻烦的。
...
## n37p15 视觉注意力递归：从良性循环到任务注意力
`CreateTime 2026.05.06`

回顾：在37141中，分析了：如果不想知道细节是什么，还想去细看吗？  
本节前段：继续探求另一个问题：我得先知道大概是什么，再去细看。
本节后段：通过流程步骤分析，得出：任务注意力的提法。
...
## n37p19 视觉注意力递归：任务注意力实践
`CreateTime 2026.05.07`

回顾：在37152中，对方案2，整个思维循环上实现视觉注意力，做了流程步骤整理，本节做代码实践部分。
说明：需在思维循环中，实现相关的：判断吸引注意力、反射反应、聚焦行为等（见37192）。
...
**37191B-广入有失控问题导致性能差: 必须限制好每一次能广入多少条。**
...
* 子问题1: 有多个微观识别结果，都有多条refPorts，所以最后一共有多少条refPorts是失控的。
  ...
  - TODO11: 把ST识别结果的refPorts也全收集起来，做个竞争 `T`。
  - TODO12: 经测发现，stModels和gtModels的定责末尾淘汰太狠，导致过滤的快没了，直接关掉ok了 `T`。
  - 结果：把广入数 & 窄出数日志打出来调试发现广入数可控了 `T`。
...
* 总结: 得先把竞争浮现跑顺当后，再回来继续做39192 `转n38p01`。

**37192-任务注意力：视觉注意力在思路循环的代码实践**
* **前提: 开始用手机摄像头做训练，它能从粗至细的识别观察等。**
...
* **结果: 等39191C跑顺利后，再回来做这个 `暂停等待`。**

****

## n37p20 特征识别优化（五）：无意注意力
`CreateTime 2026.05.11`

指日常未明确任务的，比较随意的，漫无目的的坐着视觉来回看，无明确目标的注意力行为。

37201-无意注意力步骤流程

* 步骤1：无意注意力的生成：
  - 任务：把抽象的预测生成任务（即不那么确定的，且可能很微弱的，且可能有多重具象价值感抽象而来，但确实又有的价值感）。
  - 比如：可能指向不太确定的危险，或者食物，好玩等。

* 步骤2：无意注意力的激活：
  - 激活：本身现在没什么事情（别的紧要任务），所以才会激活这样的微弱任务（根任务竞争优胜）。

* 步骤3：无意注意力的行为：
  - 行为：去探索或吸引注意力（视觉上对可能微弱感兴趣触发无意注意力任务的那个CropRect进行识别）。

* 步骤4：无意注意力的意义：
  - 意义：无意注意力虽然大致是无意的，但对视觉的训练有巨大意义。

* 总结：其实与别的任务是一致的工作流程，只是无意注意力的价值感：更微弱、不确定、多重罢了。
* 另外：像别的：“泛化价值感任务：比如好奇心”都同理（其实思维步骤都是一样的）。
* 实践：这个无意注意力，其实与“37192的任务注意力”是一回事，不用实践。