...
| 思维4部分 >> |  |  |
| --- | --- | --- |
| 1. TIP认知 | `学习` `←` `↑` | 从具象认识为抽象,构建体,感性,有什么 |
| 2. TIR识别 | `预测` `→` `↑` | 从具象识别为抽象,联想用,理性,是什么 |
| 3. TOP分析 | `制定` `←` `↓` | 从抽象向具象,联想用,感性,想怎么做 |
| 4. TOR决策 | `行为` `→` `↓` | 从抽象向具象,构建体,理性,该怎么做,顺应需求 |

> * 12整体为认知: 其中,`认知` 与 `识别` 相对,一体一用; 一感性一理性;  
> * 34整体为决策: 其中,`需求` 与 `决策` 相对,3和4的协作,异步而各完成前后半部分;
> * 13整体: 认知就是为了用来分析制定解决方案; TIP-TOP `感性向方案的转移`
> * 24整体: 识别就是为了用来预测完成解决方案; TIR-TOR `理性实,对方案的解决`
