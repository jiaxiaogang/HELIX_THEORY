...
| 210510 | 同一解决方案的重试问题 `T` |
| --- | --- |
| 说明 | 同一解决方案,首次PM时无SP,但反省类比后有了SP,却未能及时重新PM |
| 示图 | ![](assets/345_同一解决方案的重试问题.png) |
| 分析 | 方案1: 可以通过反省结果SP与`justPValues独特码`比较,如有同区,则说明发现了问题,则重置现执行中解决方案(从demand.sub删掉即可,只要不在不应期,自然会再次被取到,重执行),而不是使其失败; |
|  | 方案2: 直接重启之,重训练第3步,相当于下次遇到任务时SP再生效; |
