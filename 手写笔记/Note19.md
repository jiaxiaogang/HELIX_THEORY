...
**B组BUG:**
  ...
  3. 决策不记教训的问题: 接上BUG2,虽然清空了actions,并且每轮都会行为化,但还是在死循环,因为每次决策结果都是`吃`,然后再外层下轮转回来后,还是决策`吃`;
    ...
    * **原始问题描述:** B4投右,马上饿,在决策中发现MC_Value的M是Seem识别的0距果,导致MC直接就是一致的,所以直接决策行为`吃`了,但此时其实是远果,此问题有以下两个解决方案:
      * 方案一: MC_Value要针对protoAlg做,而不是matchAlg `不采用:因为protoAlg杂信息太多`;
      * 方案二: 1.输出`吃`后, 2.下轮循环如果更饿了, 3.要先反向反馈类比(proto远距果 和 识别的0距果), 4.并发现远距导致吃不到, 5.进尔在下次识别远果时,不能再Seem识别为0距果; `采用95%`;
