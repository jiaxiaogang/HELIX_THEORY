...
| 19051 | A组 | 目标:果可吃 `内类比,概念识别,时序识别,正向类比` |
| --- | --- | --- |
| A1 | 直投 | absA1[pos38,dis0...]->{mv+} |
| A2 | 重启,直投 | TIR_Alg识别坚果,TIR_Fo预测时序,并进行正向类比; |
...
**A组改动:**
  ...
  5. 瞬时记忆中彻底去掉parent,因为不存在绝对的空场景输入,且空场景输入后无法被识别,也会导致mModel.matchAlg和matchFo被清空等问题; T

| 19052 | B组 | 目标:远果不能吃 `反向类比,识别,模糊匹配` |
| --- | --- | --- |
| B4 | 重启,投右,马上饿,马上饿 | absA8[pos19,dis7...]->{mv-} |
