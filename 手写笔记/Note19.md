...
| --- | --- | --- | --- | --- |
| TIR识别 | 上 | 少 | 简单预测其用 | M |
| TOR决策 | 下 | 中 | 照顾所有信息 | M+P |
| TIR反思 | 上 | 多 | 全面反思评价 | 全面M |
...
3. 总结:这两个方案都略复杂，首先A1A2可能只存在一个（虽然he中目前存在两个），但he应用于现实世界时，可能只存在一个，而MC_Value中的C在图中，可能就是A3，此时其实是A3向下用模糊匹配找M特化值（距57），而不是从A57找A1和A2（方案1示图），因为P作为新节点，在网络中关联很少，如果我们把角度变一变，重新回到以C做索引上来，那么根据A3向下找到A1和A2（因为A3和A4在防重下其实是同一个节点），那么此时有两个问题，一是A1A2中有杂乱信息（图中为Pos）干扰评价（比如Pos57在汽车前面，不能吃有危险）。
