...
　　在n19015中，TOP应用mModel后，有了四种工作模式，分别为：D+、D-、S+、S-。而TOP最终是要提交给TOR做行为化的，本文将应对此变化，对TOR进行迭代。
...
| --- | --- | --- | --- | --- | --- |
| S+ | 理正 | 思->好 | 饭张口,衣伸手 | 好事将至->顺应行为 | fo.out |
| S- | 理负 | 恐->怒 | 车撞躲,狗咬打 | 坏事将至->避免行为 | sp |
| D+ | 感正 | 盼->喜 | 饿做饭,困睡觉 | 期盼好事->行为实现 | fo全部mc |
| D- | 感负 | 忧->悲 | 疼揉揉,累歇歇 | 担忧坏事->行为避免 | sp |
