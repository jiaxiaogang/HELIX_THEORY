...
**A组BUG:**
  ...
  3. A3移动直投后,生成时序为[飞,果],而联想到时序为[果,吃],二者无法全含匹配,因为此时lastItem为果,没有(吃),所以时序识别失败,导致外类比也无法进行,从而也无法类比出[dis0...]->{mv+};
    ...
    * **解决:** 在输出行为时,也进行时序预测;
  4. 时序识别,当其元素中包含概念的识别当时为seemAlg时,则会导致时序无法全含成功;
    * **原因:** 如坚果被模糊匹配为(pos207),但proto中为(pos199),导致时序识别时无法匹配,导致失败;
    * **分析:** 本质上是概念未被经历过导致,但世上不存在两片一模一样的树叶,我们不可能经历第二次概念才能识别时序成功,然后类比,所以我们要在时序识别之前,就进行概念的类比抽象;
    * **解决:** 在概念识别时,如果是MatchType_Seem类型,则直接对概念进行类比抽象;
