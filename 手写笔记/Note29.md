...
|  | 25a. 直接用空概念当抽象; |
|  | 25b. 用pid当dataSource,用index当algsType,来防重; |

| 29026 | 场景不包含Canset帧的类比-方案分析 `选用方案2` |
| --- | --- |
| 方案1 | 将AB帧的相似点抽象出来 `5%` |
...
|  | 解决: 可以根据`matchFo.pId`和`matchFo.index`来防重,这样就完美的指向了这个场景下的这一帧; |
|  | 实践: 直接用场景PID当dataSource,然后用index当algsType,即可; |
| 结果 | 选定方案2 `在29025-25a & 25b继续进行代码实践`; |
