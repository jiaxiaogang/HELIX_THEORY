...
- [回测防撞训练 & 继续觅食训练](#回测防撞训练--继续觅食训练)
  ...
  - [n29p04 共同点抽象3: `初步回测`](#n29p04-共同点抽象3-初步回测)
  - [n29p05 共同点抽象4: `在Canset抽具象基础上分析决策所需的改动`](#n29p05-共同点抽象4-在canset抽具象基础上分析决策所需的改动)
  - [n29p06 共同点抽象5: `在场景共同点抽具象基础上分析决策所需的改动`](#n29p06-共同点抽象5-在场景共同点抽具象基础上分析决策所需的改动)
...
1. 本节在最近共同点抽象大的改动后,本节初步回测: 29041本来是计划一条条测试的,但后来发现许多后期的模块是难以直白的测试的,只能边跑边发现问题,无法一蹴而就的测出有什么BUG (所以`转n29p05`边跑跑看决策需要改什么,边测试那些有改动的模块有什么BUG吧);
...
## n29p05 共同点抽象4: `在Canset抽具象基础上分析决策所需的改动`
`CreateTime 2023.04.02`

上节初步测了下没明显的闪退等大BUG了,本节继续进行应对共同点抽象后: `决策所需的改动`;
...
| 29053 | 共同点抽象增强迁移性后->决策: 分析哪里需要改,怎么改 |
| --- | --- |
...
| 29057 | 共同点抽象增强迁移性后->决策: 实践方案 `参考29053-结果` |
| --- | --- |
...
| 29059 | 共同点抽象后网络结构图分析 `因这张图,本节全部中止作废 T` |
| --- | --- |
| 示图 | ![](assets/681_通过共同点抽象改动后网络结构图分析决策改动.png) |
| ★重点 | 此图中体现出的迁移性,是因为sceneFo的共同点抽象带来的; |
| ★问题 | 而本节上面分析的迁移性,是canset自身的类比抽象带来的 (空概念absCanset); |
| ★改动 | 显然,二者功能重复了,说白了,就是上面做的canset识别类比都没啥用,应该废弃掉 `先关掉`; |

**总结: 本节站在`Canset抽具象`的基础上改决策系统,但突然发现Canset识别类比与共同点抽象功能重复,需要废弃,所以本节的实践中止,转29061在`场景共同点抽具象`的基础上进行决策改动**
