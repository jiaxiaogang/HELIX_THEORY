...
|  | 设想: 判断`烤`和`拌`有没有共同的抽象absAlg,有的话直接用来做类比结果即可; |
...
| (二) | Canset抽象部分: |
| 21. 用抽象生成新Canset | 场景包含的: 层级跟随的用matchAlg填充 `参考29023-思路1`; |
| 22. 用抽象生成新Canset | 场景不包含的: 用protoAlg填充 `参考29024-步骤3&5` |
| 23. 迭代Canset识别算法 | 将Canset时序识别算法迭代下,支持下`烤和拌的类比抽象` `参考29024-总结` |
