...
- [回测防撞训练 & 继续觅食训练](#回测防撞训练--继续觅食训练)
  ...
  - [n29p05 共同点抽象4: `改涉及决策所需的改动`](#n29p05-共同点抽象4-改涉及决策所需的改动)
...
|  | 1. F1一般可被matchPFos识别到; |
|  | 2. 用F1的具象来取F2,并做匹配度竞争 (因为F2不一定被识别到); |
|  | 3. 用S2的具象来取S3,因为S2的`躲`是空概念不可行,只有上飞才是真实可行的行为输出; |
| 结果 | 其中第2,3是新东西,需要此次分析实践; |
...
| 2 | 看能激活抽象canset并尝试求解; |
| 3 | 看调试下matchPFos的具象上,验证是否有更稳定的解; |
| 4 | 看调试下更稳定的解是含空概念的absCanset,它的具象上,又有更可行的解; |
...
| 29055 | 共同点抽象增强迁移性后->决策: 实践规划TODOLIST |
| --- | --- |
| 1 | feedback兼容下空概念; |
| 2 | 29053-说明-2: 用pFo取conPorts场景们,并与protoFo做匹配度竞争; |
