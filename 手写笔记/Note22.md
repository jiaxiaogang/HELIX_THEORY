...
- [v2.0版本六测 & 评价器 & R-模式迭代 & 防撞训练](#v20版本六测--评价器--r-模式迭代--防撞训练)
  ...
  - [n22p8 R-对比R+ 与 生物钟触发机制](#n22p8-r-对比r-与-生物钟触发机制)
  - [TODOLIST](#todolist)
...
## n22p8 R-对比R+ 与 生物钟触发机制
`CreateTime 2021.01.28`

| 22081 | R-生物钟触发机制迭代 |
| --- | --- |
| 说明 | R-的触发器,并不是ActYes来构建的,而是从预测时就构建,并开始倒计时; |
| todo1 | 开始时: R-模式触发器,从加入任务池时,就开始倒计时; |
| todo2 | 抵消时: 新的matchFo抵消后,即使旧的触发也不做反省; |
| todo3 | 行为化: R-失败时,则设demand为Failure并停止行为化; |
| todo4 | 行为化: R-成功时,则设demand为ActYes并等待触发; |
| todo3 | 触发后: 到期root有效且失败时,则反省,并移出任务池(过期认命); |
| todo4 | 触发后: 到期root有效且成功时,则反省,并移出任务池(破壁成功偶耶); |
...
| TODO | STATUS |
| --- | --- |
| 1. 因为PM逻辑复杂,所以将其拆分并融入到流程控制中,代码:`废弃P独特码,直接_Hav循环每条V调用_GL,并进行稀疏码评价,不属于独特码的本来已实现,评价为true而已`; |  |
