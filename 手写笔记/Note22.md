...
| 22081 | 预测触发器与预期触发器 |
| --- | --- |
| 预测 | 预测触发器,用于触发In反省,从tir_Forecast时就构建,并开始倒计时; |
| 预期 | 预期触发器,用于触发Out反省,从tor_ActYes行为后构建,并开始倒计时; |
| todo1 | 预期触发器:新的matchFo抵消后(root失效),即使旧的触发也不做反省; |
| todo2 | 预期触发器:破壁失败时,则S反省,并移出任务池(过期一切晚了); |
| todo3 | 预期触发器:破壁成功时,则P反省,并移出任务池(破壁成功偶耶); |
