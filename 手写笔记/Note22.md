...
- [v2.0版本六测 & 评价器 & R-模式迭代 & 防撞训练](#v20版本六测--评价器--r-模式迭代--防撞训练)
  ...
  - [n22p10 决策四模式兼容分析-deltaMv迭代](#n22p10-决策四模式兼容分析-deltamv迭代)
...
| TODO | STATUS |
| --- | --- |
| 1. 非同区同向mv,即使fo内容相同也不去重,否则其目标不明确,致其SP也不明确; |  |
| > 本来absFo指向的mv就是综合求平均的,fo不去重会导致混乱,所以先训练再说; |  |
| 2. R-模式优先以matchFos中找解决方案; | T |
