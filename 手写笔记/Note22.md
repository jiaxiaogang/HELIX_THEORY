...
| 22144 | R-需求的静默成功:`R-模式理性反省` |
| --- | --- |
| 说明 | 在时序的某帧未自然发生时,R-任务应自然完成,并反省类比; |
| 分析 | 现在对R-模式仅支持,最终的mv是否触发反省,不支持理性每帧; |
| 实践 | 1. R-决策中,每帧的推进,得先有indexAlg,再修正之; |
|  | 2. 如果_Hav()在瞬时记忆中未找到mIsC通过的curAlg; |
|  | 3. 并且等待indexDeltaTime后未自然出现,则直接静默成功; |
|  | 4. 即R-的_Hav中,第3级cHav不工作,直接生物钟触发器触发ActYes等待; |
|  | 5. OutBack成功时:`继续进行决策`,失败时:`则任务静默成功`; |
