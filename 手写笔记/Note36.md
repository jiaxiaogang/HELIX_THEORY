...
- [继续测和优化GT识别不够准确问题](#继续测和优化gt识别不够准确问题)
  ...
  - [n36p07 解决ST错位问题：迭代GT自举算法减维到GV](#n36p07-解决st错位问题迭代gt自举算法减维到gv)
  - [n36p07B 继续将：GT自举减维到SV](#n36p07b-继续将gt自举减维到sv)
  - [n36p08 GT自举减维到GV：回测GT竞争浮现（三）](#n36p08-gt自举减维到gv回测gt竞争浮现三)
  - [n36p09 迭代GT类比V8](#n36p09-迭代gt类比v8)
  - [n36p10 GT自举减维到GV：回测GT竞争浮现（四）](#n36p10-gt自举减维到gv回测gt竞争浮现四)
  - [n36p11 解决GV错位问题：迭代GT自举算法减维到切图](#n36p11-解决gv错位问题迭代gt自举算法减维到切图)
...
| --- | --- |
| TODO1 | 根据stGroup.bestGVs的bestGV_Proto切图，与bestGV对比匹配度。 |
| TODO2 | 切图可以复用ST识别算法中的切图复用缓存池。 |
| TODO3 | 竞争因子：这样改后从ST到GV的位置符合度就全是1了。 |
| TODO4 | 相当于JvBuModel和JvBuItem全不用了，完全就是assGT循环出ST再循环出GV去切图匹配判断。 |
...
| TODO1 | 从0.8,1.0,1.2缩放范围(三种) + 八个方向各偏移1.0,1.2两个值(9种) = 共27种。 |
| TODO2 | 性能：先向8个方向尝试1.2偏移把能最准的方向算出来，再在此上尝试缩放(共9+3=12种)。 |
