...
//2Q: `in类比sames` 与 `out类比sub(缺什么/渗透到algNode和微信息)`;
//2A: 把决策中foOrder的类比出 `缺的条件与行为`; (随后写)

//3Q: 在可预测的行为执行中(如正在飞行去坚果处),此时的mvCache状态如何处理;
//3A: 预测本质上就是mv基本模型,给行为方案一个状态,来挂起mvCache;

//4Q: mvCacheManager是否仅存一条需求;
//4A: 以后做,需与意识流搭配做

//5Q: TC295行updateCMVCache(),在mvCache有饿的需求时,乱投坚果,导致望梅止渴的bug;
//5A: 先判断条件满足,只需要输出行为,即输出行为,然后可去掉此mvCache;

//6Q: TC525行调用dataOut_TryOut()返回值,对mv无效时的判定及处理;
//6A: 无法直接判定,可走预测方式,参见问题4;
