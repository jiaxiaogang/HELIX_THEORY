...
| 方案1 | 追加训练: 看能否习得在不飞时,又在`偏上偏下`位置被撞到的经验; |
|  | 追加训练也不会无方向前辍,因为初始位置正中不飞就不可能`偏上偏下`; |
| 方案2 | 加上`改变小鸟初始位置`的功能 `太麻烦,不采用`; |
| 方案3 | 先飞下再直击,使之可以识别到`偏上偏下`的pFo `不用改立马见效`; |
|  | 选用此方案回测有效: `FZ46,↖飞,拉回偏上位置,直击`,但有新问题如下图: |
|  | ![](assets/602_识别偏上pFo有效但无解决方案.png) |
|  | 新问题: 虽然有效,但找不到解决方案,需要多多训练:`转26029`; |
| 结果 | 选定方案3回测ok,新的找不到解决方案的问题`转26029`; |
