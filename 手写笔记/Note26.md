...
| 26081 | 训练步骤规划 | 目标 |
| --- | --- | --- |
| 1 | `随机出生位置,扔木棒,重启` x 200轮 | 可识别到`偏上下`pFo |
| 2 | `飞或飞或直击x5,重启` x 100轮 | 学会正确方向躲 |

| 26082 | 按照26081的步骤-重训FZ50 |
| --- | --- |
| 测项1 | 识别准度提升: `偏上偏下`认知部分测试; |
|  | 结果: 在训练到1步后,测试通过; |
| 测项2 | 重训下看root循环卡顿的问题是否还有; |
|  | 结果: 没了,暂未复现,应该已经解决; |
| 测项3 | `偏上偏下`正确躲避方向,决策部分测试; |
|  | 结果: 决策过程并不顺利,需要规划新的训练步骤等 `转26083`; |

| 26083 | 训练步骤分析调整 |
| --- | --- |
| 训练 | 边直击边`上,下,左上,左下,右上,右下`飞 x 5 |
| 目标 | 1. 习得[危险地,上飞,安全地]的经验; |
|  | 2. 并能够在Solution()中,准确的取得这条经验; |
