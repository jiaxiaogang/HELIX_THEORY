...
```c
25125-工作流程 与 抽象S流程 (注: 空想流程,非代码跑出)
第1次输入F1. [路偏上0.1,上]->安全;
...
| --- | --- |
| TODO1 | 选定执行的方案在TCEffect反馈时,由累计到effectDic改成spDic; |
...
|  | > 有一帧canset在proto里没匹配到,即proto未全含canset,则整体不匹配; |
| 2 | 直至匹配到protoFo的末位为止,此时cansetFo的截点为cutIndex `T`; |
| 3 | 此时判断cansetFo在cutIndex后面还有位数,即可通过 `T`; |
| 4 | 计算前段匹配度=`匹配度总和 / 匹配数` (匹配数=cutIndex+1) `T`; |
| 第2步 | 候选集排序方式 (进一步对S进行排序择优); |
| 1 | 对cansetFo在lastMatchIndex后的部分进行SP好坏评分; |
| 2 | 候选集综合评分=`前段匹配度 x 后段SP好坏分` |
| 3 | 过滤掉`候选集综合评分` < `pFo好坏评分`的候选集; |
