...
| 原则 | 分三次竞争分别取limit,三次顺序为: 先排宽松的,再排严格的; |
...
| TODO4 | 写预测验证功能 (参考2619i-改2); |
| TODO5 | 如下图,将概念排序改成sumVNear/nearCount; |
|  | ![](assets/636_概念识别匹配度计算不公平问题.png) |
| TODO6 | 任务池竞争改成`sumSPScore/pFos数`; |
| TODO7 | 慢思考先匹配度筛选,后稳定排序; |
| TODO8 | 快思考先H>5筛选,后匹配度排序; |
...
| 26212 | 预测验证-实践规划 |
| --- | --- |
| 分析 | 分析下`预测验证`与`慢思考`的不同点,因为不同就全是可复用了; |
|  | 验证其实就是pFo转移:从抽象低稳定性,转向具象高稳定性; |
