...
- [修细节 & 训练学搬运 和 用搬运](#修细节--训练学搬运-和-用搬运)
...
|  | 2. 然后通过`问题2`跳过了R子任务,又把TCPlan简化了下; |
...
追加: 在本节训练FZ97过程中,此关联错误的BUG未复现,再观察一段时间,如果还没有,就不用管了;
...
问题: 在上表训练项1完成后,尝试进行训练项2,结果发现,试错机制似乎有点问题,一直在试错H棒(如下日志),可是试一次不就行了?它为什么没被传染掉,会一直反复试;
说明: 中间帧超时未反馈,但没传染
...
下步: 本表修复完成,回测ok,并且有了`皮果动机`,下一步回归FZ975继续训练;
```
...
| --- | --- |
| 训练项2 | 学搬运: 学会搬运(踢坚果); |
|  | > FZ975: 跑三四次`饿,等看到有皮果动机后,扔路边带皮果,踢到路上,扔棒去皮,飞至吃掉`,存为FZ975; |
