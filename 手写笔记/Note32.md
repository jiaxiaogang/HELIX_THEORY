...
- [训练和修细节](#训练和修细节)
  ...
  - [n32p10 连续视觉](#n32p10-连续视觉)
  - [n32p11 用搬运（三）](#n32p11-用搬运三)
  - [n32p11b 迭代-学Canset触发机制(更易发)](#n32p11b-迭代-学canset触发机制更易发)
  - [n32p12 HCanst学和用-知识结构整理](#n32p12-hcanst学和用-知识结构整理)
  - [n32p13 回测-学Canset触发机制(更易发)](#n32p13-回测-学canset触发机制更易发)
  - [n32p14 训练FZ97](#n32p14-训练fz97)
...
|  | 回顾: 在30152中,其实遇到过类似Canset太具象杂项太多的问题,最后的方案也是做`连续视觉`,只是当时时机尚不成熟; |
...
本节原本计划连起来跑跑,但后来因为上节`用搬运`不稳定的问题,转下节先解决`连续视觉`,然后等用搬运稳定后,再来测连起来跑吧;
...
| --- | --- |
| TODO2 | 在瞬时记忆长度限制4的规则: 相邻疑似同概念帧计做一条 (根据`相似度或抽具象关系程度`判断) `T`; |
...
| 结果 | 改完TODO2后,经回测新的保持`四条不同概念`规则ok `T`; |
...
2. 测试日志说明: flt1是生成hDemand flt2是feedbackTOR反馈到了 flt3是输出"用搬运"踢行为成功
...
| 方案3 | 直接把root->sub这一条TCPlan激活的`工作记忆线`打成一条flt日志 `本方案更直观,很简单就可实现`; |
...
本节把NewRCanset,NewHCanset,AbsRCanset,AbsHCanset四处构建,改的`学Canset更易发`;
...
```java
32115-代码段1: flt1表示每一次行为化日志 (比如下面第一条表示正在行为化R任务的解RCanset=F3991,当前行为化帧=A3955);
...
在n32p11b中,迭代了`学Canset触发机制(更易发)`,本节回测之;
...
在上节末,测得抽具象关联有错误,本节重新进行训练,观察能不能复现,重新进行FZ974之后的训练 (训练步骤参考n32p01b);

| 32141 | 在FZ913基础上接着继续后面的步骤 (参考32013) |
| --- | --- |
| 训练项1 | 测下newRCanst,absRCanset,newHCanset,absHCanset四个都能执行到 `在31183完成过,此处重训练下`; |
|  | > FZ974: 跑三四次`路下出生,饿,路上扔带皮果,扔棒去皮,手动飞至,触发吃掉`,存为FZ974; |
...
|  | > 下一步计划2: 而我们要在非理想化的情况下,也能够推进下去,所以直接跳过2和3,执行训练项4; |
| 训练项2 | 试错训练: 测下可以快速响应Canset的OutSPDic变化,使有用的canset快速具备竞争力; |
|  | > FZ975: 跑三四次`饿,无皮果,飞,吃`,存为FZ975; |
|  | > 然后测下: `测下无皮果动机`; |
| 训练项3 | 学去皮: 学会去皮(压去皮); |
|  | > FZ976: 跑三四次`路下出生,点击饿,在生成H无皮果后,扔有皮果,扔木棒去皮,上飞吃掉`,存为FZ976; |
|  | > 重点观察: 1.feedbackTOR能反馈到无皮果 2.生成扔棒去皮H经验; |
|  | > 然后测下: `用去皮: 能生成H有皮果动机`; |
| 训练项4 | 学搬运: 学会搬运(踢坚果); |
|  | > FZ977: 跑三四次`饿,等看到有皮果动机后,扔路边带皮果,踢到路上`,存为FZ977; |
|  | > 重点观察: 1.生成有皮果动机 2.学会得到有皮果的`搬运HCanset`; |
|  | > 然后测下: `用搬运: 使用搬运`; |

```java
32142-BUG: 查下传染机制是不是有问题:
问题: 在上表训练项1完成后,尝试直接进行训练项4,结果发现,试错机制似乎有点问题,一直在试错H棒(如下日志),可是试一次不就行了?它为什么没被传染掉,会一直反复试;
说明: 中间帧超时未反馈,但没传染
日志: 如下日志,A4101出现了N次,但其实它应该已经被传染了,不应该一次次的激活 (但这里却这么多次激活了);
...
回测: 发现修了TODO1和TODO2后,还是有问题;
再查: 经调试,发现未触发的原因,并不全是状态问题,因为RCanset在besting后,触发器要3s左右,而3s内它已经又变成bested了;
线索: 即,Canset并没有持续性推进(很快变成bested了),导致它没等到触发就被打断了;
  1. 有反馈时: 在前面一两节中,我们在优化学canset触发机制时,已经改了:它`有反馈时`,即使任务csStatus和bestStatus怎样,都能反馈到;
  2. 无反馈时: 本表遇到的问题是,它`无反馈时`,还是应该保持besting的,因为说白了: **没有持续推进,就不会知道它是否真的不可行**;
所以: 还是先查下,为什么它变成bested了,能不能保持其besting状态,再调试如下:
再查: 经查,每一次生成H棒后,在TCPlan中它都稳定激活了,只是这个H棒在hSolution中,都没找到解,导致这个任务立马就WithOut噶了;
...
方案3. 不管有没有解: 无论H是否无解(即使无解,变成了bested状态),在frameActYes()中,下一帧的SP和传染依旧执行 `95%`;
      > 原则: **成功有途,失败无途**,只要失败了,不必为失败找任何借口,直接计S和传染即可;
      > 方案3更直白简单,因为下一帧无论是否转为WithOut无解,或者因无解转成bested了,它都应该照样执行SP和传染`无解: 即=S,即=传染`;
抉择: 最终选定方案3,即是否无解转成bested状态,不影响下一帧的frameActYes()触发;
...
flt1 A4101(距90,棒) fromR:F5212[A4101(距90,棒),A4098(距12,果),飞↑,A4096(距0,果)]
flt1 A5069(向172,距185,棒) fromR:F5211[A5042(向84,距14,果),A5056(向171,距172,棒),A5042(向84,距14,果),M1{↑饿-16},A5069(向172,距185,棒),A5042(向84,距14,果),飞↑,A5077(向77,距6,果)]
结果: 最后只是把传染时的一些不必要状态要求取消掉了而已 (TODO1看起来必要,但最后是改了TODO3后才彻底好的);
回测: 如下日志可见,现在木棒在传染后,就不会再出现了(等待传染的期间还是有可能重复出现的,比如:A4852),等一会是可以全部及时传染掉的,并且在试错一会后,发现木棒是无论如何不可行了,它也转向找皮果 (有了皮果动机);
flt1 A4101(距90,棒)
...
下步: 本表修复完成,回测ok,并且有了`皮果动机`,下一步回归FZ977继续训练;
```
