...
| 3 | H一直在嵌套了好多层,它看起来有重复循环 (参考代码段4); |
|  | 比如: 比如找奶油做蛋糕,又找蛋糕取上面的奶油; |
|  | 解答: 在这种有循环的情况下,应该打断H嵌套 (对于重复的,不必生成子H任务,直接输出ActNo,然后尝试下一个canset); |
|  | 实践: 可以考虑到realTimeCansets实时竞争中,把curFrame已经有baseHDemand重复的,直接pass过滤掉; |
