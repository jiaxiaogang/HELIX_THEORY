...
- [测试 & 细节改动 & 训练](#%E6%B5%8B%E8%AF%95--%E7%BB%86%E8%8A%82%E6%94%B9%E5%8A%A8--%E8%AE%AD%E7%BB%83)
  ...
  - [n18p16 精细化训练](#n18p16-%E7%B2%BE%E7%BB%86%E5%8C%96%E8%AE%AD%E7%BB%83)
...
| 4 | **重启,远投,马上饿** | T |
|  | 生成A6(g255,size5,pos19,dis7)->{饿} |  |
|  | =>识别失败,因从未见过dis!=0 |  |
| 5 | **马上饿** | T |
|  | 生成A7(g255,size5,pos19,dis7)->{饿} |  |
|  | =>A6类比A7,抽象出absA8(g255,size5,pos19,dis7)->{饿} |  |
|  | 解释: 因为瞬时不清空,所以A6A7所在时序都含A6,故抽象出A6->{饿} `转TODO_Q3` |  |
|  | 注: 日志中,显示识别成功了,回头查下为何? `TODO_Q4` |  |
| 6 | **重启,远投(投向与A6不同的方向),马上饿** |  |
|  | 生成A9(g255,size5,pos23,dis9)->{饿} |  |
|  | =>识别失败,因从未见过pos23,dis9 `转TODO_Q1` |  |
|  | =>A9类比A8,抽象出absA10(g255,size5)->{饿} |  |
|  | =>BUG_TIR有场景和坚果,两个概念需要被识别,第一个识别A10成功,第二个长度却变成0 `TODO_Q5` |  |
| 7 | **远投** |  |
|  | 生成A11(g255,size5,pos24,dis12) |  |
|  | =>识别matchA = A10(g255,size5) |  |
...
| TODOLIST | DESC |
| --- | --- |
| Q1 | 识别部分,如果模糊匹配是否可以识别为A6?原先先match后fuzzy的方式,是否不妥? |
| Q2 | 识别fuzzy部分,如果模糊匹配支持多个稀疏码,就可以成功识别为A9;此处需看下代码是否已支持,如不支持,需迭代支持下; |
| Q3 | 在饥饿mv发生时,是否将内存时序A7进行持久化?否则一重启立马又忘了,自己上次怎么饿的; `目前暂时不动,后续再考虑`; |
| Q4 | 在不该识别成功时,识别成功了,需查下细节原因; |
| Q5 | 估计这里就是前面概念长度为0的大BUG来源; |
|  | 经查: 概念长度为0的bug竟然与matchAlg2FuzzyAlg()算法有关,两个识别循环第一轮结束后,调用了此方法,第二轮就直接成0了; |
