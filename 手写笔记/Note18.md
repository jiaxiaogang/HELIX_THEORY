...
| 4 | **远投** |
|  | 生成A6(g255,size5,pos19,dis7) |
|  | =>识别失败,因从未见过dis!=0 |
| 5 | **马上饿** |
|  | 生成A6->{饿} |
| 6 | **远投** |
|  | 生成A7(g255,size5,pos23,dis9) |
|  | =>识别失败,因从未见过pos23,dis9 `转TODO_Q1` |
| 7 | **马上饿** |
|  | 生成A7->{饿} |
|  | =>抽象出absA8(g255,size5)->{饿} |
| 8 | **远投** |
|  | 生成A9(g255,size5,pos28,dis12) |
|  | =>识别matchA = A8(g255,size5) |
|  | =>识别fuzzyA = A7 `转TODO_Q2` |
...
| TODOLIST | DESC |
| --- | --- |
| Q1 | 第6步识别部分,如果模糊匹配是否可以识别为A6?原先先match后fuzzy的方式,是否不妥? |
| Q2 | 第8步识别fuzzy部分,如果模糊匹配支持多个稀疏码,就可以成功识别为A7;此处需看下代码是否已支持,如不支持,需迭代支持下; |
