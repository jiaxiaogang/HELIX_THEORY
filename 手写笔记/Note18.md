...
| 4. 测下,在TIR中,能否对[远果,吃],产生长时时序记忆; |  |
| 5. 测下,能否形成[远果,吃]->{焦急}; |  |
| 6. 测下,在理性行为化分析中,[远果,吃]并未指向{mv+},导致理性判定为远果不能吃; |  |
...
> 在上节末,对外循环中焦急时机分析,导致远果不能吃,不能够以焦急来做感性评价,而转向理性判定`远果不能吃`,所以本节主要针对此常识的学习以及使用展开,分析理性时序[远果,吃]->{N}的形成,以及使用其进行评价`不能吃`;
> ##### 符号说明:
> 1. ->{N}: 表示fo未指向mv;

* ##### 18077_学习`远果不能吃`常识 (学习过程分析)
  1. TIR,形成内存fo1[远果,吃]
  2. TIP,偶尔形成fo2[远果,吃]->{更饿mv-}
  3. 第一步: 考虑,对fo1的持久化;
  4. 第二步: 考虑,对fo1的抽象 (形成确切的不能吃);
  5. TODOTOMORROW继续...

* ##### 18078_使用`远果不能吃`常识 (判定不能吃)
  1. 前提: 形成[远果,吃]->{N}(未指向mv)后;
  2. 判定: 在行为化中,进行理性判定,远果不能吃;
  3. 分析: 不能吃在理性上判定有两条,一是无法吃为(果无),二是无法解决{mv+};此处采用第二种;
  4. 代码: 取到fo[远果,吃]未指向mv,故判定无法解决饥饿问题;
