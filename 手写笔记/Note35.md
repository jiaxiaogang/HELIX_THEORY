...
|  | TODO1.1: 必须对protoRect分组成索引key，比如每80%比例分一个wh粒度组，然后每过0.8偏移分一个xy偏移组。 |
|  | TODO1.2: 宽高比从0.1到10，每1.3倍分一组（如：1:1一组，1:1.3一组，1:1.69一组）。 |
|  | TODO1.3: 宽度从1到最大(maxW)，每1.3倍分一组（如：宽3一组，宽3.9一组，宽4.797一组）。 |
|  | TODO1.4: X从0到最大(maxW)，每宽度单元的20%为一组（如在3.9组里每0.78一组：0-0.78为一组，0.78-1.56为一组，1.56-2.34为一组）。 |
|  | TODO1.5: Y从0到最大(maxH)，每高度单元的20%为一组（如在宽=3.9且1:1.3组里每1.014为一组：0-1.014为一组，1.014-2.028为一组）。 |
|  | TODO1.6: 以上四个分组索引index，构建一个4维字典，取四个key，然后一级级取出最终对应的缓存。 |
| 方案2 | 自举结果优化：对bestGVModel的防重再改进，原来用gvId_ProtoRect的Key太具体了，改成一个范围记同一个key。 |
|  | TODO2.1：类似TODO5.1生成分组切片key，然后依此对bestGVModel防重，即把protoRect分组key，依此直接取最终结果返回。 |
