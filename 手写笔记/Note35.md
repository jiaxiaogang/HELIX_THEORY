...
| 方案3 | 减少调用次数，给每次调用增强复用池，即第二次调用如果还是相近的值，则直接复用相近值的旧识别结果。 |
|  | TODO3.1: 切入v值按精度1%来复用，即如果值域内有1000个值，每10个index下标归为一组。 |
|  | TODO3.2: 以此类推，复用池内最多有100个条目供复用，比如index在320-330区间复用同一个识别结果。 |
