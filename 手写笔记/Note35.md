...
| 方案3 | 把一切ST识别过程可复用的复用之：protoGVIndex复用，即每次从protoDic切图，改成切图区域相似90%以上的直接复用。 |
|  | > 实践方式1：要么循环判断哪些区域匹配90%以上（计算量大些，不过准确度高）。 |
|  | > 实践方式2：要么直接把精度降低做KEY（比如宽高93，精度10%计为9，x=18计为2）（这个精度内的直接取这个KEY复用即可）。 |
|  | > TODO3.1：先采纳实践方式1，如果不行再考虑2 `T`。 |
...
| 方案6 | ST识别结果中很多ST重复：比如同样是ST347就占了28%。 |
...
|  | > TODO6.1: 判断两次st的pid一样，且whxy相近，则允许合并处理。 |
|  | > TODO6.2: 把每个assIndex对应的gv合并复用（只保留最好的一条）。 |
|  | > TODO6.3: 以及即使不是最好的gv，也把其rect记录下来，用于防重（即每个rect只要失败过，直接判为失败）。 |
|  | > TODO6.4: 没失败过的，则可以与best二者PK一下。 |
|  | > TODO6.5: 现代码中的bestGV是用别的方式在竞争的，看现在这个是与之融合代码，还是单独再写一块bestGV竞争代码。 |
...
|  | > TODO6.6: 计算整个assSTAtProtoRect，然后对比rect相近度，决定是否对两个assST进行合并。 |
