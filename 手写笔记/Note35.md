...
| TODO1 | 每次局部特征识别后，把切入rect的切入gv_p存下来。 |
| TODO2 | 再次局部特征识别时，把新切入rect和gv_p与之前的缓存中求rect相近度（交/并=近）。 |
| TODO3 | 大于70%则直接复用结果（其实相当于直接跳过此轮识别）。 |
| TODO4 | 小于70%则进行该轮识别，并将识别结果再次缓存到当前rect和gv_p中。 |
