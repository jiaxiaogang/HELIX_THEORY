...
| 35072 | 组特征改回独立网络模块实践规划：TODOLIST |
| --- | --- |
| TODO1 | 从ST向GT识别时，也要用到位置符合度，比如0的两竖，就是位置符合度为左右两边。 |
| TODO2 | GT.content由ST_p组成，还是由ST.gvs组成？按直观来看，几乎肯定是应该由ST_p组成。 |
...
|  | 方案：先由ST识别结果的前10条，组成GT.content，如果以后发现这样不太行，再深入改进该方案。 |
| TODO3 | GT识别不能用自举，因为显著特征的相对位置可能会有变化，在protoDic中切图，往往切不准确。 |
