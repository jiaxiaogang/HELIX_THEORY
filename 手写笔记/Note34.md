...
| TODO1 | 方案4-把GV构建时，改成先转成三索引值，然后将索引做为V进行装箱。 |
| TODO2 | 方案4-然后再将三索引值做为GV.content装GV的箱。 |
| TODO3 | 方案4-这样GV构建时也是根据三索引值来防重。 |
| TODO4 | 方案2-先收集最终全含有效，再进行相近度乘积竞争等操作（写个和特征识别类似的模型，先收集后竞争，最终输出识别结果）。 |

| 3416x | 更多的可能的优化方案（以后再说） |
| --- | --- |
