...
| 优化3 | 现在的性能还很问题，得用性能debug工具，过一下性能，该优化的优化下。 |
...
|  | 性能4B: 但仍慢，仅靠最内层加`取交防重`是不行的，判30w防重都吃不消，得每一层都尽量加上防重 `T`。 |
|  | 性能4C: 为每一层都加上cache，统一设计下加到瞬时序列的ShortModel里，只要ShortModel不销毁或被新short使用，继承过来 `T`。 |
|  | 总结：各种防重和缓存机制都加上了，虽然也有很大效果，但治标不治本，即使只循环8w次，也得2秒多 `转34074 T`。 |
...
| 缺点 | 无法区分1-2-2-3-3-3-4-4-5和1-3-3-3-3-3-3-3-4=5的区别，因为二者完全可以排成：顺序，平均值，差值，都一样。 |
