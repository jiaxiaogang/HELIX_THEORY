...
- [测试推进细节修正的问题，并整理推进(觅食 & 飞躲 & 踢搬运)三项的多向连续训练,以及三项融合训练](#测试推进细节修正的问题并整理推进觅食--飞躲--踢搬运三项的多向连续训练以及三项融合训练)
  ...
  - [n34p09 多码特征的：从特征抽象 到 特征嵌套 到 竞争浮现](#n34p09-多码特征的从特征抽象-到-特征嵌套-到-竞争浮现)
...
| TODO1 | 方向：改为先分暗和明，然后二分界方向。 |
| TODO2 | 差值：可以考虑用max-min值，不过最好是用二分界的平均值一求差。 |
| TODO3 | 均值：值的计算仍是9值求均不变。 |
| TODO4 | 以上分别建索引，共三个索引，注意方向索引是loop首尾相接的。 |
| TODO5 | 组码识别改V3版本，改为三个索引乘积计算匹配度。 |

***

## n34p09 多码特征的：从特征抽象 到 特征嵌套 到 竞争浮现
...
| 34091 | 从 特征抽象 到 特征嵌套 |
| --- | --- |
| 1触发抽象 | 每次概念识别都进行概念类比，提升特征抽象性。 |
| 2习得抽象 | 从类比出关键特征（比如眼睛，鼻子，眼神，嘴角，表情等）。 |
| 3使用抽象 | 到识别关键特征，提升识别准确度（可识别交层特征，不要求全含）（比如单亮点，或长长的胡子弯线）。 |
| 4特征嵌套 | 同时识别到多个抽象特征时，判断其位置符合度：即`根据一个protoT识别到多个局部特征assT`的位置符合度。 |
