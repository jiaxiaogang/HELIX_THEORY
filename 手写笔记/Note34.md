...
|  | 4、根据位置相符度v2算法 和 复用匹配度等，识别到准确的assGT结果。 |
...
| 方案4 | 特征识别assTs后，再向conPorts取具象整体特征（并判断位置相符度，抽具象局部特征匹配度），得出结果：做为更准确的特征识别结果。 |
...
| TODO1 | 先掐头去尾，计算平均缩放比例，做为统一缩放比例。 |
| TODO2 | 统一缩放后，再掐头去尾，把平均偏移量算出来，做为统一偏移对齐数。 |
| TODO3 | 把整体特征assT的每个局部absT，都进行先按TODO1的结果缩放，再按TODO2的结果偏移，以实现与protoT对齐。 |
| TODO4 | 以proto为准，分别计算对齐后的scala相近度，deltaX相近度，deltaY相近度（离proto最近的为1，最远的为0）。 |
| TODO5 | 将三个相近度乘积，求出综合匹配度。 |
