...
| TODO1 | canset父非子: 所有I和F层的SP各计各的,不继承 (包括迁移时,也不初始继承spDic,及今后F的增强,全不同步到I层); |
| TODO2 | 当I受到SP反馈时,相当于F也受到反馈,同时累计SP+1; |
| TODO3 | canset池竞争时,采用匹配度的牛顿冷却曲线来计算综合评分,用于竞争; |

| 33115 | 问题: **父非子的公式是什么?** |
| --- | --- |
| 说明 | 本表,用于分析`子非父`的综合评分公式; |
| 原则 | 原则是: F从客观量上看是80%,但对I的作用力上变成20%; |
| 方案1: | 冷却作用力,作用于spScore |
...
| 问题 | 冷却公式的`时间进度`怎么算? |
|  | 线索: 不能直接用匹配度当时间进度,因为匹配度可能都很高,比如多个F全部都在90%,此时就会导致I缺失自我; |
|  | 解答: 应该先以匹配度对F进行排序,然后用排序的index,做为它的`时间进度`; |
| 抉择 | 环境对个体影响力: 1.在个体成长前应该有80% 2.在个体成长后应该有20%; |
|  | 线索1. 为了区分个体成长程度,就必须用spStrong,因为率无法表示成长程度,比如: 当个体S0P1时,直接100%稳定,它的率最初就达到了满分; |
|  | 线索2. 冷却公式已经给了个体成长空间,它已经有了表达优势,比如: I层的P100表示的就是100,而F的100可能只表示3; |
