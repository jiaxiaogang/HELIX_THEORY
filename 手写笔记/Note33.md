...
| --- | --- |
| 名词 | 子R任务: 这种反思识别预测到的是子R任务,它是与父任务链标识都不相同的子R任务 (父链上,可能有多个不同标识的R任务); |
...
| TODO1 | 子R任务要挂在baseCansetFo下; |
|  | 作用: 为了下次反思时复用等方便&也为了通过root.baseOrGroup说明它是子R任务; |
| TODO2 | 子R任务也要挂在loopCache中; |
|  | 作用: 为了在TCPlan中,做root竞争时,它能够同台竞争; |
| TODO3 | 如果触发了重新反思,那么旧的几条子R任务要先从loopCache中移除掉; |
| 疑问转折 | 如果base没激活呢?子R任务还有和别的root同台竞争的机会吗? |
|  | > 答: base都不激活了,子R任务自然也就不应该激活了; |
|  | > 如: 翻山遇老虎危险,突然想到还有条水路可走,自然就不必再准备猎枪了; |
| 结果 | 根据以上疑问转折,子R任务还是挂在baseFo下即可,不必加到loopCache下与root同台竞争 `转33072继续`; |
...
| 本表计划 | 在上表中,发现子R任务就是子R任务,它做不得root,那么相当于我们还是回到老以前的做法,把它当子R任务即可; |
| 思路 | 需要把TCPlanV2改进下,再让它支持子R任务; |
| 步骤1 | 如果子R任务非常严重(反思评价不通过),比如翻山必然会遇到老虎,那么直接就评价不通过scoreNo掉了; |
| 步骤2 | 如果子R任务没那么严重(反思评价通过),比如翻山百分之一可能遇到老虎,才可能冒着风险翻山; |
| 步骤3 | 为子R任务求解,以及行为化等...... |
| 步骤4 | 子R任务求解时竞争出best时,要先评价下它是否来的急 (避免父任务很急时,子任务却在慢悠悠的跑); |
|  | 展开: 但像拍一下篮球这种小任务,每一个动作都反思下尚可,但R子任务再求解下,确实来不及 `转33074`; |
| 步骤5 | 子R任务的解,只需要在out帧时输出行为,in帧时就不必管它,但有反馈时,也要接受反馈,以及继续推进cutIndex+1; |
| 步骤6 | 子R任务的解有推进时,说明子R任务不那么严重了,要削弱它的任务评分,而不是像以前的root那样加强; |
| 步骤7 | 子R任务的进度,和父任务的进度,二者之间需要并进; |
|  | 比如: 还没找着枪,父任务就不能出发翻山,必须等它先找到枪,分析过程如下: |
|  | 展开: R子任务的前提是父任务,它不必做预测 (只要父任务的推进,就是子任务跟着推就行) `转33073` |
|  | > 时序1. 父时序是: `[翻山]`; |
|  | > 时序2. 子R任务是: `[翻山,见虎]->{危险}`; |
...
| 步骤8 | 其实不用分析这么细致,目前的小鸟训练,能做到激活和求解子R任务就可以了,太细的思量,压根测不到,需求不明确; |
| 注意 | 不能沉迷在推进子任务解决中,要及时将思绪拉回父任务 `转33073-优点 & 33074-优点 T`; |
| 总结 | 以上通过步骤分析了启用反思子任务,另外`在33073和33074有两个伴随问题的展开分析`,另外`实践转33075`; |

| 33073 | 启用反思子任务后的: 伴随问题1 - R子任务`不预测`但可反馈 |
| --- | --- |
...
| 33074 | 启用反思子任务后的: 伴随问题2 - R父子任务的`来不及`矛盾 |
| --- | --- |
...
**小结: 以上33072通过步骤分析了启用反思子任务,并且在33073和33074分别展开分析了两个伴随问题,下表可以做实践了;**

| 33075 | 启用反思子任务 -> TODOLIST |
| --- | --- |
| 说明 | 目前小鸟演示用不着太复杂的设计,实践上简单支持下即可; |
| TODO1 | 在TCPlan中,每一层一个fo战胜后,优先推进它的R子任务; |
|  | > 回顾: 以前TCPlanV1时就是这样,现在也可以回去参考下以前TCPlanV1的代码; |
| TODO2 | 在TCAction中,每推进一帧,都要重新触发反思识别R子任务一次,原来旧反思的全删掉重存一下; |
|  | > 正据: 因为子R任务也要随时根据父任务的推进来响应变化,所以父任务每帧推进,都要重新反思一下; |
| TODO3 | 当子R的canset行为化中的帧,与场景有映射时,即使它是行为输出帧,也不输出,静默等待它反馈即可 (参考33072-步骤6); |
|  | > 回顾: 以前有过`防止弄巧成拙`的代码,在写这个之前,可以先参考下有没有用; |
| TODO4 | 子R任务在求best解时,先根据父任务给的时间,判断下是否来的及 (参考步骤4); |
|  | > 回顾: 以前有过`时间来不及`评价器代码,现在应该还在,在写这个前,可以先参考下有没有用; |
