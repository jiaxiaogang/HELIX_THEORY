...
| --- | --- |
| 问题 | 留意下: `行为化反思`的重组功能,用不用改下,总不能每一帧推进,都行为化反思识别一回(这太耗能); |
...
| 33069 | secondRefrection() 二筛反思掐头 |
| --- | --- |
| 所以 | 说白了,两个反思是一样的,只是在初筛时,不调用反思识别算法,best后进行二筛时先调用下反思识别; |
| 线索 | 两个反思可以直接合并成一个,也不需要初筛了,毕竟即使初筛过了,行为化时还是做二筛,不如直接合并成一次筛选; |
| 流程 | 在这个思路上推演一下反思的工作流程: |
| 步骤1 | 当反思通过时,直接行为化canset,当反思不通过时,中止找best结果,直接生成派生root子任务,并等待下轮TO再跑; |
| 步骤2 | 当派出子root任务 |
| 步骤2 | 当二筛不通过的canset,它的派生root任务,又随着canset推进几乎已经被解决时,初筛也可以复用反思识别结果的子任务; |
| 步骤2 | 此时二筛不通过的canset,又可以通过了; |
| 步骤3 | 两个反思算法可以统一一下代码 |
