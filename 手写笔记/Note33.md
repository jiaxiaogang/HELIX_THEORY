...
复现: FZ992,饿,附近扔坚果;
疑点1. 是因为没有习得准确的解?它的映射全错误了,映射到了末帧?
  > 查下: 是否需要手动引导吃到几次,看下日志,为什么映射到了末帧?
疑点2. 会不会是因为末帧的SPScore太容易是1分了,导致全是它?毕竟等着就行,只要成功过一次,就是1分;
  > 查下: 把这些末帧的直接过滤掉,看激活的是不是就正常了?
