...
|  | 所以: 当子R的canset行为化中的帧,与场景有映射时,即使它是行为输出帧,也不输出,静默等待它反馈即可; |
...
| TODO3 | R子任务的前提是父任务,它不必做预测 (只要父任务的推进,就是子任务跟着推就行) `参考步骤7 转33073 T` |
| TODO4 | 但像拍一下篮球这种小任务,每一个动作都反思下尚可,但R子任务再求解下,确实来不及 `参考步骤4 转33074 T`; |
| TODO5 | 不能沉迷在推进子任务解决中,要及时将思绪拉回父任务 `转33073-优点 & 33074-优点 T`; |
| TODO6 | 当子R的canset行为化中的帧,与场景有映射时,即使它是行为输出帧,也不输出,静默等待它反馈即可 (参考步骤6); |
|  | > 回顾: 以前有过`防止弄巧成拙`的代码,在写这个之前,可以先参考下有没有用; |
| TODO7 | 子R任务在求best解时,先根据父任务给的时间,判断下是否来的及 (参考步骤4); |
...
| 结果 | 目前不必做太细的设计,实践上简单支持下即可 `转33072-TODO6 T`; |
...
| 结果 | 以前就有过`时间来不及评价器` `转33072-TODO7回顾下以前的代码,看需要改的话改下启用起来 T` |
