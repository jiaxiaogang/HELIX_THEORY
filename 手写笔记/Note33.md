...
- [(觅食 & 飞躲 & 踢搬运)三项的多向连续训练,以及三项融合训练](#觅食--飞躲--踢搬运三项的多向连续训练以及三项融合训练)
  ...
  - [n33p15 H迁移延着R迁移关联进行-细节修正](#n33p15-h迁移延着r迁移关联进行-细节修正)
...
//总结：求H解时：
1、H求解，只在IF树内进行（参考6.1）。
2、要延着R迁移关联进行H迁移（参考8.TODO4）。
3、可传染整个工作记忆树的actIndex帧（参考8.TODO2）。
4、当前IF树内无解时，通过左突右进来实现从无到有（参考7A&7B）。
```
