...
| 问题 | F1`[我在5B,车开到5B]`如何识别为F2`[我,车开过来]`? |

| 25073 | 方案2代码规划 |
| --- | --- |
| 1 | 视觉感官算法不能再区分view,而是整屏所有的dic整合到一起输入; |
| 2 | 复查下algRecognition算法,有全含结果时也要支持seemAlgs结果; |
|  | 比如:看到某物飞行像蚊子,会联想到蚊子飞行路径,但此时我并没看到蚊子; |
|  | 分析: 上实例佐证全面放开seemAlgs,并且本节局部匹配后续流程需要它; |
| 3 | 概念识别中matchCount迭代为matchValue相似度 `计入稀疏码值相近度`; |
|  | 现状: 以前完全按匹配数量matchCount来计算的; |

| 25074 | 迭代TIR_Alg算法,获取更多的局部匹配结果 |
| --- | --- |
| 问题 | 现在的TIR_Alg算法,仅有matchCount匹配度,没有matchValue相似度; |
| 分析 | 需要分析下,根据稀疏码相近值,来做相近稀疏码匹配,再做局部概念匹配; |
| 方案1 | 微向宏: 从索引序列中取相近的value,根据refPorts取到别的具象局部匹配概念; |
|  | 这其实是另一种意义上的全含,只是有一个值不一样; |
|  | 但有可能相近的数个值,都取不到什么具有高抽象refPorts的相邻码; |
| 方案2 | 宏向微: 从seemAlgs中,找出同区不同码的,根据值排序,取出最相近的; |
|  | 这可能出现较多低质量的结果,这些结果可能都不是相近的; |
| 方案3 | 从抽象向具象找`含内区不同值`的具象概念,并值排序; |
