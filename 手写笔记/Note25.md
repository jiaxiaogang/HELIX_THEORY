...
| 方案 | 可设定脚本,编辑脚本后,可以指定跑某脚本,来达到强化训练目标; |

| 25146 | hSolution半途而废的BUG |
| --- | --- |
| 问题 | ![](assets/586_hSolution半途而废的BUG.png) |
| 复现 | 25133-`FZ39,直击,下飞,直击,重启,直击,下飞`...大致是这步骤; |
| 方案1 | 新增飞后视觉 `5%`; |
| 方案2 | 加飞后视觉,本质上是为了及时反馈,可以考虑改为每秒3帧被动视觉 `95%`; |
| TODO1 | 将视觉内所有View独立生成Dic改为全部生成一个; |
| TODO2 | 将鸟自身的一些关键数据(如位置)也加入到视觉中 (还可避免空帧); |
