...
| BUG1 | `载入FZ36-6,执行第7步直击`,发现R任务的两个解决方案全在危险地带? |
|  | `F186[A1(高100,Y207,X2,距121,Y距35,向←,皮0)]-> FRS_PK评分:0` |
|  | `F4[A1(高100,Y207,X2,距121,Y距35,向←,皮0)]->M1{↑疼-9} FRS_PK评分:0` |
| 怀疑1 | 经查,在FZ36-5左右时,发现即使安全地带偏击,也会反省S,而不是P; |
| 怀疑2 | 训练3-6步中停,导致危险地带却不被撞,从而危险地带许多P; |
|  | 验证: 可对危险地带取rSolution时打出SP日志验证猜想; |
|  | 经查: SP全是0,中停啥也没导致?全是0?`需要明天继续进一步查`; |
|  | 解决: 可调整训练步骤,不中停 `转25112`; |
