...
| 25131 | FZ39BUG_RSolution取到坏经验 |
| --- | --- |
| 示图 | ![](assets/Note25-71d4080e.png) |
| 说明 | 如图F442为-mv经验,但它的index2下SP为0分,所以负但不顺利,评为1分; |
| 问题 | 如图取F442做解决方案有些怪异,分析与解决思路如下: |
| 思路1 | 可考虑排除-mv经验 (因为-mv的导致越努力结果越差); |
|  | 分析: 如取F442为解决方案,则会推进它,到index2反而把1分推成0分了; |
| 思路2 | 可考虑把0分的排除 (都没成功过一次,直接pass省得浪费时间); |
| 思路3 | 考虑加大训练力度,来增加SP的经历 `参考25124`; |
