...
| A学习 | 理性学习: 如`[↗飞]`预测了危险,就和非全含mFo类比学习,即片面还常发; |
|  | 感性学习: 有mv指向,无论匹配度是多少,感性学习都ok; |
|  | 结论: **理性学习需要筛选出全含的**,感性学习本次无需代码改动; |
| B预测 | 理性预测: 全含的已全发生,不存在预测; |
|  | 感性预测: 有mv指向,无论匹配度是多少,感性预测都ok; |
|  | 结论: **理性预测需要筛选出非全含的**,感性预测本次无需代码改动; |
| C需求 | 新增需求: 有mv指向,无论匹配度是多少,新增PR需求都ok; |
| 方案2 | 综上,对`理性学习`和`理性预测`重新制定取prFos的规则,如下: |
|  | **1. 筛选: 理性学习筛选全含 & 理性预测筛选非全含;** |
|  | **2. 排序: 理性学习应以fo更长排序 & 理性预测应以匹配度排序;** |
