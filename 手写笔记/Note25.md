...
| 25239 | 回测-FZ42训练步骤 |
| --- | --- |
| 1 | 随机飞,扔木棒,循环5次 x 5轮(每轮重进成长页,不重启); |

| 2523a | FZ42强化训练后期,energy不更新的问题 |
| --- | --- |
| 说明 | 在FZ42训练后期,所有新的R,全已在短时记忆中出现过,而`未形成需求`; |
| 例如 | 路上遇到一百辆车,就要躲一百次; |
| 方案1 | `未形成需求`时,也要更新energy `5%`; |
|  | 分析: 同一任务每推进一步,就重复赋值energy不太好; |
| 方案2 | 旧有finish和failure状态的rootDemand不防重 `95%`; |
|  | 分析: 这条易实现,且保持了`每root任务更新一次energy的方式`; |
