...
- [乌鸦挑战-TC数据流配置测调 & 迭代反思TCRefrection](#乌鸦挑战-tc数据流配置测调--迭代反思tcrefrection)
  ...
  - [n27p17 抽象多层多样性之: TO复用相似度](#n27p17-抽象多层多样性之-to复用相似度)
...
|  | 缺点: 要这么处理下,最好没什么性能问题,比如mIsC优先用抽象取具象来达成; |

indexDic应该也需要持久化,并且事实上它最重要,包括:
1. 找出cutIndex
2. 重新计算matchValue都可以根据它来进行;
3. Solution快慢思考pk时,重新计算时序的相似度;
	> 需要取出absFo和conFo,才能够根据它们的contant_ps来计算,所以比如canset有70条,那么就要取70条IO
	> 不过即使如此,也比现在的比对要性能好,现在的比对是完全将每条alg又取出来,再比对的;
