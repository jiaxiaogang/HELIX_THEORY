...
- [乌鸦挑战-TC数据流配置测调 & 迭代反思TCRefrection](#乌鸦挑战-tc数据流配置测调--迭代反思tcrefrection)
  ...
  - [n27p15 抽象多层多样性之: 整体规划 & 相似概念抽具象](#n27p15-抽象多层多样性之-整体规划--相似概念抽具象)
  - [n27p16 抽象多层多样性之: 迭代类比器](#n27p16-抽象多层多样性之-迭代类比器)
  - [n27p17 抽象多层多样性之: 调整决策系统](#n27p17-抽象多层多样性之-调整决策系统)
  - [n27p18 抽象多层多样性之: 回测](#n27p18-抽象多层多样性之-回测)
...
## n27p17 抽象多层多样性之: 调整决策系统
`CreateTime 2022.10.25`

抽象多层多样性调整认知期(相似概念抽具象)后,也相应需要调整决策系统,本文即主要做这部分;

| 27171 | 大致本节任务分析 |
| --- | --- |
| 1 | 在决策中,复用此次改动matchDic的相似度; |
|  | > 在TCSolution中,对AIAnalyst的相似度计算,改为复用matchDic; |
|  | > TODOTOMORROW: 回顾下TCSolution的代码,看需要改哪些代码; |
|  | > TODOTOMORROW: 直接看下AIAnalyst中的几个方法,看分别哪里调用,怎样复用; |
| 2 | 因为网络结构抽象多样性变化,相应调整决策系统; |
...
|  | 分析1: 可以尝试通过A:B1,然后A:B2两个值综合计算出相似度; |
|  | 比如: 我想吃饺子(A),具象中有一堆各样饺子(B1,B2...); |
|  | 分析2: 因为本次的相似度抽具象关联,所以具象B...中,总有抽象的,有相似度值的; |
|  | > 那么我们是否应该以那些抽象的有一定优势? |
|  | > 即,此问题在于: 调整了抽象多层多样性后,反思中的竞争机制应该怎样相应调整? |
