...
- [乌鸦挑战-TC数据流配置测调 & 迭代反思TCRefrection](#乌鸦挑战-tc数据流配置测调--迭代反思tcrefrection)
  ...
  - [n27p20 抽象多层多样性之: 回测](#n27p20-抽象多层多样性之-回测)
  - [n27p21 三段抽具象](#n27p21-三段抽具象)
...
| --- | --- |
| 方案1 | feedbackAlg时,所有actionIndex到targetIndex之间全是wait状态; |
| 方案2 | 此时,并不是有一帧有反馈,而是父级任务有完成的; |
|  | 这个完成触使它的子solutionFo提前完成,触发类比; |
| 情况1 | 像action到target间全是wait状态, |
|  | 比如我找张三和李四回家,先找到李四了,张三仍然需要找; |
| 情况2 | 父级... |
