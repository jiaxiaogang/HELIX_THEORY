...
- [乌鸦挑战-TC数据流配置测调 & 迭代反思TCRefrection](#乌鸦挑战-tc数据流配置测调--迭代反思tcrefrection)
  ...
  - [n27p18 抽象多层多样性之: TO调整决策系统](#n27p18-抽象多层多样性之-to调整决策系统)
  - [n27p19 抽象多层多样性之: 回测](#n27p19-抽象多层多样性之-回测)
...
| 分析 | 在决策时,许多不完全时序会影响到决策 (可能行为化半天浪费感情); |
|  | 因为: 不安全时序可能只是暂时安全,也可能结束时真的安全 (不确定); |
|  | 1. R帧是不完全时序,最好在真的结束时再类比 (触发器触发时结束); |
|  | 2. P帧是完全时序,但一样危险,只是找出了危险的抽象规律; |
|  | 所以: 要避免不完全时序的类比,仅对完全时序进行类比; |
| 结论1 | 识别时几乎是不完全时序,它不应建立抽具象关联; |
| 结论2 | 可将识别时的indexDic暂存在inModelManager.indexDDicCach中,等类比时复用; |
|  | > 查下P帧类比的P帧protoFo,是否有indexDic缓存? |
|  | > P帧在曾经不安全的时候,识别到同区mv的matchFo,是否也应参与类比? |

***

## n27p18 抽象多层多样性之: TO调整决策系统
`CreateTime 2022.10.25`
...
在抽象多层多样性推进到类比复用indexDic时,发现类比中许多时序是`不完全的`,而这些不完全时序是不应该类比的,全节针对这一问题进行改动,算是插入到`外层多样性`之间的需求;
