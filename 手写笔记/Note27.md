...
|  | 结果: 所以选择方案1 `转思路3`; |
| 思路3 | 是否对失效任务,做主动改为finish的机制; |
|  | 方案1: 如果原本能预测到的危险,现在预测不到了,是否使其失效? |
|  | 方案2: 如果原任务在反省时,发现已过期,那么使其失效; |
| 思路4 | 使用可视化和强化训练等工具,导致我对单轮日志细节的测试不足; |
|  | 分析: 其实每次调整代码后,我都应该先测单轮日志,再测跑强化; |
|  | 总结: 先单轮测试和测反思,然后测大概ok了,再跑强化,测强化效果; |
...
| TODO6 | 无论是否循环卡顿,都不影响先测下TCRefrection反思功能; |
