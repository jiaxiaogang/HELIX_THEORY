...
| --- | --- |
| 规划 | 对tirAlg的全含结果进行refPorts联想,查看其是否价值明确; |
| 1 | 如果明确可直接返回识别结果 `->识别成功`; |
| 2 | 如果不明确,如价值时正时负,则进行以下操作: |
|  | a. 根据matchAlg结果中独特部分的指引 (如距8,pos9等...); |
|  | b. 在具象中,如果找到明确的结果,如有距果总是不能吃,则返回结果 `->识别成功`; |
| 注 | 因为matchAlg的独特稀疏码可能多条,所以最终明确的结果,也可能是多条; |
