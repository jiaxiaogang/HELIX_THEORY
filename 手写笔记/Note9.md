...
| 思维对思维的欺骗 |
| --- |
| 只有思维`相信自己思考的是最真实的对象`;这样的话;当你思考到某些角度的某些细节的时候;只有这个角度的这个细节会展示给你;我不必将所有细节进行建模及渲染(性能考虑);而只设计一个`自由的编辑器一样的想像力引擎`; |
...
| 技术实现 | 想像力不必使一切与现实渲染一致,但保持一致可以使思维与行动更加一体,所以还是要尽量(目的与性能之合)设计一致; |
| 技术目标 | 想像力是为多联想事务服务的,所以只要达到 `最佳多事务` 为目标即可; | 
