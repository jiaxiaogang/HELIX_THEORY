...
| 作用栗子 | 其在he4o软件架构中的作用;就像以往软件编程里编译器的角色;但其不能够将所有数据进行编译,也不能够将逻辑编译为确定 |
| 注: | 意识如此神秘,在本系统中似乎无处不在,但却又无迹可循,就像与我们捉迷藏一样,无论你如何深入的思考,意识总是那么神秘,本章只能够以软件架构技术的方式来设计与实施 |
| 是否具有意识? | 欺骗he4o,让其认为自己有意识 |
***
...
| 2 | 电脑cpu内存硬盘是分离的,所以在he4o设计上必须思考这种异同,来设计he4o软件架构 |
...
| ---------- | --------------------------------------------------- |
| 能量位     | 增加抑制信号集成到事务控制器中...                   |
| 能量位原理 | 针对数据量来运算,而非连接强度;(如:属性占0.1,obj占1) |
...
|  | title | desc |
| --- | :---:| --- |
| 1 | 简介 | 想像力在he4o中是`代码能力`,其融合一个或多个`联想`事务;并通过思维将其逆向演绎 |
...
| 6 | DOP的控制器 | 以往控制器;用来写model,view等其它层逻辑;而he4o中,控制器不仅是控制自身的代码顺利运行,也要作为一个监督者的身份;来作数据的元操作与控制数据的安全范围; |
| 7 | 定义 | 在he4o中把这种安全范围的控制称作:"规则"; |
