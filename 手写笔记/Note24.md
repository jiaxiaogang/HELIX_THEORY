...
| --- | --- |
| 复现 | `FZ27,直击` |
| 回测 | 问题依旧,S经验不足,在加训后,依然不足; |
| 分析 | 怀疑dsFo进入PM时,毕竟dsFo本就是解决方案,自然P(顺)多S(逆)少; |
| 分析 | 尝试将R任务的rFo加入PM分析中,因为rFo预测生成任务,自然S多P少; |
| 验证 | 1. 从生成SP代码处,分析验证,以上分析猜想是否成立; |
|  | 2. 再从rFo中,读出S数P数,进一步验证是否S多P少; |
| 方案 | 将rFo作用于PM中 (dsFo取P? rFo取S?) (验证通过后,此方案生效); |
