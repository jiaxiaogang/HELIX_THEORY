...
| 多码协同 | 如果方向也多变呢?无论多少个码,多多变,都依下面同样的代码规则跑; |
| 多码协同 | PM对独特码逐一评价,失败时,继续尝试下一条,全失败时才失败; |
| SP表征1 | IRT和ORT的SP无论是否有必要,先分开分别表征SPStrong; |
| SP表征2 | SP仅描述`好坏`两个强度值,不再描述具体的稀疏码值; |
| SP表征3 | 抽象F的SP不再具体描述它不包含的V值; |
|  | > 比如:F1就仅描述自己SP,F2也是仅描述左向SP,不描述Y距的; |
| SP表征4 | 新增一种SP表征方式,将SP像deltaTimes一样存在foNode中; |
| SP表征5 | SP仅对自己打分,即仅在fo中表征sStrong和pStrong两个值即可; |
