...
| 实践 | 1. IRT和ORT的SP可能要分成不同,不能再共同使用SP; |
|  | 2. 抽象F的SP不再具体描述它不包含的V值; |
|  | > 比如:F1就仅描述自己SP,F2也是仅描述左向SP,不描述Y距的; |
|  | 3. SP仅描述`好坏`两个强度值,不再描述具体的稀疏码值; |
|  | 4. 取消束波求和,因为SP不再描述具体稀疏码值,束波求和不需要了; |
|  | 5. VRS评价不再依赖束波求和,而是依赖F下的`SP强度`做稳定性判断; |
| 扩展 | 1. 左右向木棒,识别为两个不同场景; |
|  | 答: 可行,即取到的F1本就更稳定些,但依然得向下找稳定F; |
|  | 2. 从F1出发,找Y距稳定时,会不会找到F3并中止? |
|  | 答: 所以当F3不包含Y距同区码时,需要判断它是否... |
