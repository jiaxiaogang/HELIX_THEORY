...
- [防撞训练](#防撞训练)
  ...
  - [n24p14 迭代R决策模式-RAcion.Fo行为化](#n24p14-迭代r决策模式-racionfo行为化)
...
|  | 1. 关门时,转子任务,解决方案为:将门打开; |
|  | 2. 堵门时,转子任务,无解决方案,递归找别的路(正门); |
| 结果1 | 无时不在反思,正在行为化中fo的元素被瞬时输入mIsC命中,即立马反思; |
| 结果2 |  |
...
分析下,我们什么时候反思子任务,什么不反思
1. (主任务经常做的话,往往不需要反思),
2. 当飞下觅食,并不是反思到危险地带,而是看到有车过来,才反思到危险;
	> 即反思,其实是当前正要做的事,与瞬时记忆相配合重组,

3. 反例: 当出门穿越森林,反思到有老虎,此时并没有看到森林,但也可以反思到危险;

什么时候新看到的,要与反思间协作 (比如,我要走侧门,看到侧门关着,立马改道走正门);

即: 瞬时输入的与正在执行的fo中元素有mIsC匹配时,即代入重组,并反思;

而已经经历多次走侧门未走通,此时SP比例显然是<1,如0.8左右,此时并没有新的标识mv,所以并没有引起反思子任务;

0. 短时中无时,直接反思它的mv评分 [穿越森林,老虎危险]->mv-99疼死
1. 短时中无时,直接以反思它的SP评分 [走侧门]->80%能行通,
2. 短时中有时,直接重组时序,并反思 [走锁上的侧门]->行不通;
