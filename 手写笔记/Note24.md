...
| 24151 | 代码实践分析2 |
| --- | --- |
| 架构 | 重写`solution,action,regroup,reflect`四个方法; |
| 防重 | 针对整个任务树防重,补充,更新等 (参考24142); |
| 运行 | 四个方法相互调用,螺旋运行; |
| 流程 | 如果action成功,等待外界反馈后regroup; |
|  | 如果action失败,则跳转....明日继续分析后面cHav和反思子任务; |
|  | 参考24143,R任务->解决方案->反思->子任务->行为化->递归; |
| 途径1 | 父任务: 组合时序->预测R任务->解决方案->行为化; |
| 途径2 | 子任务: 重组->反思->解决方案->行为化; |
| 途径3 | 行为化输出: 反馈重组->反思... |
| 途径4 | 行为化跳转: 子行为化->子解决方案->子重组->子反思... |
