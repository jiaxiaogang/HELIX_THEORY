...
- [防撞训练](#防撞训练)
  ...
  - [n24p07 PM迭代V4-更理性](#n24p07-pm迭代v4-更理性)
...
| 24072 | PM迭代V4: 代码实践 |
| --- | --- |
| 单码评分 | 取消束波求和,因为SP不再描述具体稀疏码值,束波求和不需要了; |
| 单码评分 | VRS评价不再依赖束波求和,而是依赖F下的`SP强度`做稳定性判断; |
| 多码协同 | 无论多少个码,每个都很多变,都依下面同样的代码规则跑; |
| 多码协同 | PM对独特码逐一评价,失败时,继续尝试下一条,全失败时才失败; |
| 多码协同 | 如果一个码没有任何重复的,那么SPStrong就永远只有1或0,怎么办? |
|  | 分析: 比如苹果在手边,什么距离算手边?这样的概念如何形成? |
|  | 如: F1:`[伸手,可及]`的映射,即老早以前提过的时序的概念化; |
|  | F1不包含距离,它仅表征伸手动作可得到的结果,它的具象fo才有距离; |
| SP表征1 | IRT表征感性SPPerceptStrong,而ORT表征理性SPReasonStrong; |
| SP表征2 | SP仅描述`好坏`两个强度值,不再描述具体的稀疏码值; |
| SP表征3 | 抽象F的SP不再具体描述它不包含的V值; |
|  | > 比如:F1就仅描述自己SP,F2也是仅描述左向SP,不描述Y距的; |
| SP表征4 | SP仅对自己打分,即仅在fo中表征`感理性spStrong`四个值即可; |
| 初始强度 | 具象SP初始为1,抽象SP初始强度为具象之和; |
| 应用联想 | 从所有rFos&pFos出发(含自身),向具象找稳定SP; |
| 应用联想 | 如何防止找到F3这种叉枝,做为"稳定"结果?`rFos&pFos最具象判断?` |
| 应用联想 | 空S评价,也要跟着改动,考虑废弃它,因为抽象切入不负责稳定性; |
