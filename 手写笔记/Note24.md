...
| 第3阶段 | 废弃dsFo |
| 1 | 生成R任务时,同mv标识只生成一个任务; |
| 2 | R模式从pFos抽具象排序,从抽象开始取做解决方案; |
| 3 | 在行为化中不需要再反思,_Fo和_Alg走走过场; |
...
| BUG2 | 示图-4: Y距35并非最佳修正目标,最佳的应该是pPorts中束波评分最高的; |
|  | 分析: F335的pPorts仅一条,即评价不通过时,需排除同区码pFos; |
|  | 1. 评价时,从更全面找出哪个最稳定 (不论好坏); |
|  | 2. 修正时,找出最稳定的好实现横向场景跃迁 (也是从束波中找); |
|  | 如果整个pFos都评价为否,并且都包含同区码呢? |
|  | 越抽象,越可能提供这种双横向分支,所以先向着抽象找,F335找到F334,然后再从F334看是否不包含同区码,再评价其束波曲线,找出峰值P; |
|  | 为了不脱离场景,修正码就从SP找,绝对不从横向跃迁到别的场景; |
|  | 1. 所以就先评价时,全面找最稳定; |
|  | 2. 修正时,从每个不含同区码里,找P/(P+S)峰值最大的,如果多个为1,则选择绝对值P最大的; |
