...
| 2 | **从同层场景取解决方案** |
|  | 例: 坏开关,切至60%时可开灯,但这个60%很难学会; |
|  | 而60%时,开关会嘎一下,这个时序   Fo1: [切开关,嘎一下]->{mv+} |
|  | 此时,Fo1并不在识别的rFos和pFos中,但却在当前任务的rDemand.matchFo的具象下; |
|  | 即,R任务取解决方案,应该不限于"抽具象路径"下,而是可以从conPorts下取同层 (进行修正加工); |
| 3 | **优先从哪取解决方案** |
|  | 在抽具象路径上: |
|  | 1. 越具象,SP经验也越少,但可以贴合场景:比如开关嘎一下时即可好; |
|  | 2. 越抽象,SP经验也越多,但可能脱离场景:比如破坏开关; |
|  | 综上: 应当优先从具象,找不到稳定P经验时,再向着抽象尝试; |
