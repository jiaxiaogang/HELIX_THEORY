...
- [防撞训练 & TC新螺旋架构](#防撞训练--tc新螺旋架构)
  ...
  - [n24p19 螺旋架构-solution决策路径](#n24p19-螺旋架构-solution决策路径)
...
| 结果 | 因为TCSolution.solution()是从root出发的,需要全树pk`转24184`; |
...
| 问题 | **如何选出最优路径?** |
| 1 | 同层间,Solution多选一,而Demand全算数 `转24192`; |
...
| 代码 | solution()需要整合二者工作,即判断当前是否有子任务待决策; |
...
## n24p19 螺旋架构-solution决策路径
`CreateTime 2021.12.13`

在上节中,已分析了综合评分,并且最终分析solution的模型得出三条原则;
1. 制定了从root出发,HDemand理性淘汰,感性淘汰,感性竞争的三步大致模型框架;
2. 末枝评分优先原则: 综评以sub迭代base (末评);
3. S选一R求和原则: 同层间Solution多选一,RDemand全求和;
...
| 24193 | solution决策过程-纵深度与横广度哪个优先 |
| --- | --- |
