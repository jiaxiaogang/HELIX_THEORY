...
| 24162 | 整体架构图设计 |
| --- | --- |
| TO路径 | TO路径并非单条,有actYes,isOut输出,转移等路径如下: |
| 虚实结合 | actYes->feedback:`普通Fo末帧结束 & H末帧结束 & 静默等待`; |
| 实 | isOut->input:`isOut=true进行输出后`; |
| 虚 | cHav->jump:`isOut=false转cHav跳转后`; |
