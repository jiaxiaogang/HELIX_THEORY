...
- [螺旋调教 & TCSolution前段条件满足迭代](#螺旋调教--tcsolution前段条件满足迭代)
  ...
  - [n28p15 回测连续飞躲,ok话回测觅食训练](#n28p15-回测连续飞躲ok话回测觅食训练)
  - [n28p16 解决飞错方向BUG & 觅食训练](#n28p16-解决飞错方向bug--觅食训练)
...
1. 本节连续飞躲改进,识别准确度提升,但还不够,下节继续;
2. 本节觅食训练规划了大概,但还没实践,也转下节继续;

***

## n28p16 解决飞错方向BUG & 觅食训练
...
| TODO | 把第2步训练的步骤连飞3下改成两下,把飞的速度变慢 `T`; |
| 回测 | 回测几次第二步,看能不能提高各向飞躲都顺利的训练成功率; |
