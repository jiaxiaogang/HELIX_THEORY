...
| --- | --- |
| 1 | I反思和生成subRDemands代码不变 `只是不求解,转2` `T`; |
| 2 | TCPlan中不将subRDemands做为可选项 (即R子任务不求解) `T`; |
| 3 | `行为化飞`及`飞后视觉木棒`的再时序识别也不变,允许新任务自由竞争 `T`; |
...
| 思路 | 查下TCScore,为什么第二次明明才-1.8分,却可以战胜第三次; |

| 28137 | BUG_卡顿循环: 如果输出再被时序识别,会触发各种行为 |
| --- | --- |
| 说明 | 因为目前的行为只有飞,所以此BUG导致出现卡顿循环不断飞,循环如下: |
| 循环 | 循环说明: 因为飞被时序识别话,又要触发任务,然后触发S,然后各种行为; |
