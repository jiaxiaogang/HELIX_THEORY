...
- [螺旋调教 & TCSolution前段条件满足迭代](#螺旋调教--tcsolution前段条件满足迭代)
  ...
  - [n28p14 调整RootDemand竞争机制: 实现连续飞躲](#n28p14-调整rootdemand竞争机制-实现连续飞躲)
...
## n28p14 调整RootDemand竞争机制: 实现连续飞躲
...
| 示例 | 张三打站立的猛虎(任务A),然后一耳光扇倒老虎(行为B),但按在地上打失败了(任务B); |
| 思路 | 此时,任务A其实已经过期了 (当我们看到老虎躺地上时); |
| 线索 | 其实,并不是任务过期了,而是前段更多了...是否后段也可以判断下是否已条件满足? |
