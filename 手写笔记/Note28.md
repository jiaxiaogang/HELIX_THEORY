...
- [螺旋调教 & TCSolution前段条件满足迭代](#螺旋调教--tcsolution前段条件满足迭代)
  ...
  - [n28p08 使S竞争越来越准](#n28p08-使s竞争越来越准)
...
| 原则1 | **前段每帧的alg在对应的matchAlgs中的排名,来替代匹配度竞争;** |
| 原则2 | **中后段还未发生,是无法判断匹配度的;** |
...
| 28084 | 前段竞争-代码实践 |
| --- | --- |
| todo1 | 在compareCansetFo()中计算前段时,执行过滤和计算竞争值; |
| todo2 | 竞争值用`衰减强度值*衰减匹配度`来计算出竞争力值; |
| todo3 | 在AIRank中,单独写前段竞争器,并过滤仅保留前20%; |
