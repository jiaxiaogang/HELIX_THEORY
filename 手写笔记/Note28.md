...
| 综合 | 本文三个重点: 1. 生成期; 2. 竞争机制; 3. 使用期; |
| 结果 | 生成迁移期实践转28188,竞争期实践转28189,使用迁移期实践转2818a; |
...
| 28188 | 迁移Canset-生成期实践 |
| --- | --- |
| todo1 | 写个teacherCansetsEffectDic,即存canset也存EFF `T`; |
| todo2 | 更新conCanset时,同步到抽象matchFo的teachers中; |

| 28189 | 迁移Canset-竞争期实践 |
| --- | --- |
| todo1 | 每条迁移Canset记录,要绑定其内特征码相近度; |
| todo2 | 根据每个特征码的相近度值,判断出哪个不重要; |

| 2818a | 迁移Canset-使用期实践 |
| --- | --- |
| todo1 | 迁移门槛高: 迁移使用的Canset,必须有更高的惰性期; |
| todo2 | 迁移来的方案,要打压一下权重; |

**小结: 28185-2818a这几张表,主要解决Canset迁移问题,在惰性期实现后,HE安静下来了不再易动乱动,但一冷静下来,发现各场景间的交流变少了,Canset随着场景的不交流,也迁移性变的差,即距离30的经验,很难过度到距离40也能用,这里解决Canset迁移问题就是解决它们之间的Canset交流,并且最终制定了在当前网络结构的基础上,以抽象为师,具象为生,这种师生传递Canset经验的方案;**
