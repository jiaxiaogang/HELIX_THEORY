...
| 分析2 | 也有可能这识别结果很正常,只是要分析一下,能不能辨别出这事实上是一点点危险可能性; |
|  | > 即在安全位置时,危险可能性不是90%左右,而是接近0%; |
|  | > 即,可能需要辨别出哪些细节是重要的,哪些不重要,更来理性的判断相似度; |
|  | TODOTOMORROW20230116: 分析下,怎么针对不同特征占比; |
|  | 关键: 只要无法越来越准,就会导致很多问题,比如这次在决策时判断前置条件满足时就出现因为这里而不准的情况; |
