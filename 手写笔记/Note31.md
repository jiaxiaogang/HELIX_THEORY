...
| 测试项1 | 重训练下看能不能触发: 构建NewRCanset和快速过渡到交层AbsRCanset; |
|  | 结果1: 在FZ934x3基础上,训练了一次步骤,即可生成NewRCanset,这个容易触发; |
|  | 结果2: 在FZ944x3基础上,训练了一次步骤,即可生成AbsRCanset,这个只要激活了rSolution结果,就很容易触发; |
| 测试项2 | 重训练下看能不能触发: 构建NewHCanset和快速过渡到交层AbsHCanset; |
|  | 结果3: 在FZ934x3基础上,训练了一次步骤,看到feedbackTOR匹配日志,也建了两条NewHCanset; |
| 测试项3 | 测试RCanset有皮果反馈后,能不能继续推进下一帧; |
