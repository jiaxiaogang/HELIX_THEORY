...
| 回测 | 测下31176末尾:`循环尝试和传染`的问题是否确定:已修复; |
...
| TODO1 | feedbackTOP在负反馈时,对工作记忆的所有任务子任务,只要判断其canset已末帧,且任务的mv指向同区,则传染; |
| TODO2 | 如果已末帧,且frameActYes未负反馈,则传染整个工作记忆中,将所有mv同区指向的重生; |
| TODO3 | 在rSolution初始化Canset池时,如果初始化直接就是末帧,且mv指向同区,也继用下传染状态; |
