...
| 31022 | 同质任务重复太多次执行问题-实践前问题: 到底是只继承oldDemand.sub枝叶,还是两帧直接并成一帧? |
| --- | --- |
...
| 31023 | 同质任务重复太多次执行问题-实践规划 |
| --- | --- |
| todo1 | 判断旧新任务的pFo一致,则直接移植旧pFo的solution枝叶给新的; |
| todo2 | 分析下旧的solution移给新Root时,是不是有cutIndex等需要兼容下; |
| todo3 | 移植后: 旧的应该过期了,应该以新的为准; |
|  | 1. 即推进一帧后,上一帧的pFo可以直接关闭 (即移植是移动,而非复制); |
|  | 2. 但上一帧也不是完全无用,有些正在推进中的canset是可以继续搞的 (即移植后,由新pFo继续推进它); |

| 3102x | 正式开始做: 步骤8-搬运训练 |
| --- | --- |
| 第1步 | 试下能必现搬运动机 |
