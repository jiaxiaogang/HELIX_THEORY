...
|  | 结果: 说白了,H反馈要写个时序识别算法嘛,然后通过重写"时序识别的全含算法"来解决问题2 `转31066`; |

| 31066 | 方案3-用反思识别评价H反馈准确度: 实践规划TODOLIST; |
| --- | --- |
| 简介 | 本表: 大概就是把原来的识别算法升级改成符合此方案要求的: H反馈评价算法; |
|  | > 本表回顾了`cansetFo识别算法` 和 `TCTransfer算法`,并参考制定此次实践todolist; |
| TODO1 | 再打开cansetFo识别算法 (在3014b-方案5中,因为`猜测尝试`,认为它没必要废弃了); |
|  | 说明: 现在要新写的H反馈评价算法,和以前的cansetFo识别算法相似度高,在它的基础上改写比较好; |
...
| TODO2 | 直接设计好写个全含算法,使之有迁移indexDic映射的,直接计为有: 关联,并且以此来取匹配度计算; |
| TODO2.1 | checkFoValidMatch全含算法支持传入content_ps而不是直接传fo,相当于在不实际迁移的情况下判断全含; |
| TODO2.2 | 参考TCTransfer代码,支持伪迁移,(不迁移,但能够获取迁移后的content_ps),还可以获取到spDic等数据; |
| TODO2.3 | 写个模型,原cansetFo,typeI/B/F,迁移后的content_ps()方法,迁移后的spDic()方法; |
| TODO3 | 时序识别最终果也收集这些模型,并以原本solutionFoRankV3输出结果顺序为依据判定满意度; |
