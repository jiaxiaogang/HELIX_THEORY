...
| 正据1 | 前面多次提过: "决策中,过度抽象是正确的废话,尤其是行为化必须是具体的" (候选集也是用多具象,而非抽象); |
| 正据2 | 具象canset的SP计数是很准确的,但抽象的未必准确 (因为它受具象累计的影响,具象可行,抽象未必可行); |
| 正据3 | 抽象father层其实也不需要优待,它应与i和b公平竞争,只要它参与竞争且SP分高了,它才是真才实料的准确可行; |
