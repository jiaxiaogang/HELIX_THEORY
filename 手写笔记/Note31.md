...
| 方案1 | protoA和cansetA,分别与交集,计算平均匹配度,二者相乘得到综合匹配度 `20%`; |
|  | 分析: 此方案可能受干扰,感觉不如方案2; |
| 方案2 | 交集中的每一条,分别与protoA和cansetA取出两个匹配度,并相乘,找出相乘值最大的,做为最终匹配度 `95%`; |
|  | 分析: 此方案算是很单纯的匹配度竞争了,且没有方案1的问题排除了干扰,且没有方案3的问题没有越界; |
| 方案3 | 每次反馈时进行反思识别分析一下满意度 `5%`; |
|  | 示例: 比如看到碎砖,用来当锤子不满意; |
|  | 步骤1: 将每次反馈代入原rCanset生成regroupFo; |
|  | 步骤2: 并从rCanset候选集中识别 (候选集 = solutionFoRankingV3.rank()结果); |
|  | 步骤3: 以识别结果在sort中的index做为满意度 (越靠前越满意); |
|  | 缺点: 此方案的缺点在于,它超出了当前正在执行中的rCanset的范畴,这是致命缺点; |
|  | 结果: 否掉,超出当前rCanset,不符合开发原则,也会让问题变的复杂; |
| 结果 | 根据以上全表分析,方案2应该比较合适,可暂用试下 `T`; |
