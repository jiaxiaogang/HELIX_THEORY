...
| 31179 | 上表末测试: 测得问题如下 |
| --- | --- |
| 问题1 | Canset[饿,饿,饿]这种硬扛饿的方案竟然排在第一名,并且在只发生了一次饿的情况下,它的行为化actionIndex直接就是3; |
|  | 1. 硬扛发现没用,自然就负EFF+1,排不到第一名了; |
|  | 2. 至于actionIndex是3,因为目前的时序识别算法就是从后向前匹配的,所以会首先匹配并映射到最后一帧饿; |
