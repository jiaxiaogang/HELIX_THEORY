...
|  | 2. 出门可能遇老虎有危险 (遇虎时) |
|  | 3. 打虎违野生动物保护法 (遇虎后) |
| 说明 | `遇虎前`和`遇虎后`都应生成子任务 `ds为:改为发声假枪 & 仅吓跑`; |
|  | `遇虎时`与父任务本来就是同一个任务,不应再生成为子任务; |
| 问题 | 是否判断ARSTime的cutIndex,以仅>cutIndex的做父子同任务防重? |
