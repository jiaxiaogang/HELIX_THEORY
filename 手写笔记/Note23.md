...
- [九测(融合训练)](#九测融合训练)
  ...
  - [n23p14 TOM中的复用机制](#n23p14-tom中的复用机制)
...
| 说明 | 如图示,在决策TOM树中,有许多重复的区块,那么我们应该考虑如何复用它们; |
...
| 分析 | 1. 在R-取dsFo时,如果dsFos已在TOM中有成功的,则复用; |
|  | 2. 任务不用做复用防重,dsFo要做复用防重; |
|  | 3. 有可能dsFo只做部分复用,如钉钉子的例子; |
