...
- [九测(融合训练)](#九测融合训练)
  ...
  - [n23p09 十测-防撞训练&防撞觅食融合训练](#n23p09-十测-防撞训练防撞觅食融合训练)
...
| 举例 | 一根很`催饿火神虫`飞过来,多条预测mv如下: |
|  | 预测mv: `1.会饿`,`2.会伤`,`3.烧伤`,`4.咬伤`,`5.烧伤皮`,`6.咬伤肉`; |
|  | 预测分析: `12最抽象`,`34略抽象`,`56最具象`; |
| 分析 | 1. 例预测中,1,2应该生成子任务,其它不生成; |
|  | 2. 例中,3456这类具象预测,也应对子任务解决方案有帮助; |
|  | 3. 各个子任务,应该将具象构建任务,并尽量找综合的抽象解决方案; |
|  | 4. 即只要抽象的一条子任务解决了,那么其下的子任务全被解决; |
| 方案 | 做去重策略,比如同mv类型`如疼`做防重; |
