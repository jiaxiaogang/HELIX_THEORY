...
| 补充 | todo: 本表改了reasonOutRethink()生成hCanset,随后看整理下代码改成reasonInRethink()生成rCanset; |
|  | > 生成rCanset要求推进完全后再调用,而目前在pushFrameFinish()调用是正合适,不用改 (参考27183); |
|  | > 如果非要整理代码对齐,倒是可以考虑把hCanset的生成时机,参考rCanset生成来改下 `不过现在不动了,后再说`; |
...
## n30p14 去皮训练-学会去皮部分4 (整理Canset演化流程 & cansetAlg反馈改为有共同抽象即匹配)
...
|  | 复现: `FZ873,路下出生,饿,路中鸟X并齐扔带皮果,等预测更饿并想得到木棒h任务时,扔木棒`; |
...
| 结果 | 本表的问题1和问题2,在30151的回测中有交待已解决 `转30151 T`; |
...
| 3014a | 30148-todo1改后,回测canset类比不顺利 (类比次数为0); |
| --- | --- |
| 回测说明 | 在pushFrameFinish中,生成新canset,然后识别结果全是0条,因为所有候选集全是最具象时序; |
| 分析 | 要考虑下类比抽象的冷启动,经调试,发现从未触发过`Canset再类比`的代码,可以先查下原因; |
| 问题1 | **先分析下,普通节点的冷启动是怎么做的?** |
|  | 答1: alg识别的结果仅有似层(并建立抽具象关联),这相当于冷启动了; |
| 问题2 | **普通alg识别面向全网,是有些泛的,可能会导致落实到场景内后,很难正好撞上;** |
|  | 分析1: 对于场景包含帧,既然当时识别到了pFo,所以pFo场景中包含的帧肯定是被识别到了; |
|  | 分析2: 对于场景不包含帧,确实有可能太泛撞不上,可以实际调试看日志证实下; |
|  | 线索: 所以场景不包含帧的匹配才是问题点: **canset中的场景不包含帧,很难在canset识别时匹配到问题** |
|  | 示例: 当任务是找锤子却没有,去找锤子时,很难找着canset里匹配度很高的那个锤子; |
|  | 思路: 找个差不多的就行,指望碰运气找着匹配度高的是不现实的 |
|  | 答案: 可以在场景内做alg识别,这样即能匹配到,又不会要求太高 `转方案1`; |
| 问题3 | **pushFrameFinish()是有单独写了个`RCanset再类比`调用的,但为什么没执行过呢?** |
|  | 流程分析: 通过整理现在代码跑的流程,分析下为什么没触发`canset再类比`; |
|  | 流程整理: 如果反馈顺利->则能触发`RCanset再类比`,但rCanset最初都是最具象的,所以不可能反馈顺利; |
|  | 代码调试: 按状态来说是TOModelStatus_ActYes状态,应该是能触发此功能的,但最终没触发,明天打断点重现查下原因; |
|  | 分析: 反馈后更应该是反省sp的范畴,并且在pushFrameFinish中,也有`生成新canset并识别与类比`的流程,所以单独调用`RCanset再类比`显多余; |
|  | 废弃: 并且按普通节点类比来看,`新canset并识别与类比`才是正途,本问题所在的`RCanset再类比`功能可考虑直接废弃掉; |
|  | 结果: 问题3可以暂不解决,优先解决问题2走正途,问题2修复并生效后,问题3问题本身可以被直接废弃干掉 `暂不修复,等待废弃`; |
| 问题综合 | 问题2提到的:`新canset识别类比`是正途,所以优先根据问题2,来制定方案如下: |
| 方案1 | 最好是能支持`场景内alg似层识别`,从而避免`问题2导致很难撞上的问题`,进尔使canset时序识别能成功,类比能成功; |
| ↓ | 实践规划: 写canset场景内alg识别算法: 其实和普通alg识别算法没啥匹配,仅仅是把"识别区域限定在场景内"而已; |
| ↓ | 缺点: 如果一个场景内的canset刚迁移过来,只有这一个canset呢?怎么识别?如果在场景内识别,那岂不是100%能识别到? |
| ↓ | 结果: 以上缺点指出了此方案有套娃问题:在场景内识别,像是让你匹配到,你就肯定能匹配到,这不是废话么 `否掉`; |
| 方案2 | 复用普通概念识别的结果,不过不过滤 `只是不过滤的matchAlgs`; |
| ↓ | 缺点: 不过滤可能很泛,从极左(场景不含cansetAlg匹配不上)走向极右(因准确度过低cansetAlg过度轻易匹配上); |
| 方案3 | 考虑打开交层识别结果,这样交层canset迁移后,它的cansetAlg也能匹配上; |
| ↓ | 缺点: 且不说是否采用此方案,此方案并不解决`冷启动`问题(即一开始cansetAlg都是似层,是先匹配到才有的交层); |
| 方案4 | 尽量用很类似的两次经历: 触发同一场景下生成类似的rCanset方案,以此来顺利冷启动canset识别和类比; |
| ↓ | 缺点: 要求有点高,会让HE太娇气,HE的学习不应该苛求外界太有序,这是不现实的; |
| 结果 | 本表来看,但可考虑用方案2或4来做冷启动,后面介入示例推演下可行性 `转下表`; |
| 追加结果 | 在3014b分析出了更好的方案5,此处四个方案的可行性都不好,全废弃; |

**小结: 最近几张表中匹配不上的问题困惑,都是源于3014a-问题2中,关于场景不包含帧的匹配问题,且3014a-给出了canset场景内alg识别等四个方案,但都不理想,转下表继续;**

| 3014b | 本表通过举例来推演下3014a的方案2和4的可行性 -> 最终得出方案5:猜测尝试; |
| --- | --- |
| 回顾 | 上表4个方案都不好,建议从更底层思考下: **canset的向性是下,可完全摒弃类比抽象的路子**; |
| 举例 | 张三要吃苹果,李四找不到苹果看到香蕉也会问下张三,香蕉行不行? (香蕉显然不是苹果,只是二者都是水果); |
| 原则 | **1. 智能体只是希望方案能推进,而不是想要有多高的匹配度;** |
|  | **2. 根据一用一体原则,决策是用为主,体为辅,所以在决策期应当避免类比抽象,应该尽量复用认知期构建的抽具关系;** |
| 思路 | 有没一种可能,cansetAlg是在尽可能的具象 |
|  | 1. 因为它的向性是下,所以尽可能的具象有利于产出更有效的canset (**抽象指导性,具象可行性**); |
|  | 2. 此时判断共同抽象匹配是ok的 (**共同抽象即猜测与尝试**); |
|  | 3. 此时假如共同抽象的猜测尝试后,发现可行(SPEFF+1) / 不可行(SPEFF-1); |
| 方案5 | 猜测尝试方案: 通过共同抽象匹配,来猜测尝试一条absCanset出来; |
|  | 比如: 此方案在上例中我们会生成: 给李四吃香蕉 和 给李四吃水果两条canset; |
|  | 优点1. 这么做有利于canset更大几率能推进 (通过猜测尝试); |
|  | 优点2. 这么做就不需要"类比抽象canset"了,只需要通过尝试的结果SPEFF来直接构建新的absCanset即可; |
| 结果 | 方案5不错,转下面两张表推进实践改代码; |

| 3014c | 方案5-第一部分: `猜测尝试`相关的实践 |
| --- | --- |
| 说明 | 本表主要分析`猜测尝试`方案需要改哪些代码; |
| todo1 | 判断cansetA反馈时,可以根据cansetAlg的共同抽象来判断 `T`; |
|  | 1. 因为feedbackTOR()的waitModel有R和H两种类型,而又可以分为过程帧和目标帧,所以分析下此任务覆盖全些: |
|  | 2. 但R的每一个过程帧都会变成一个H目标帧,所以只要遍历到所有HDemand和RDemand即可支持所有cansetA的反馈判断; |
|  | 3. 现在就是这么做的,所以不必为了覆盖全面而改什么代码; |
| todo2 | 废弃cansetFo识别和类比 `T`; |
| todo3 | 在SPEFF时,要遍历一下共同抽象cansetAlgs,都分别生成新的canset并SPEFF+-1计数 `需再深入想下,转3014e T`; |
|  | > 回顾: 以前有个TCRethinkUtil.spEff4Abs()方法,就是把抽象canset也顺便更新了SPEFF |
|  | > Q:那现在要改动什么呢? A:现在要改为只有共同抽象部分所生成的absCanset才更新SPEFF; |

| 3014d | 方案5-第二部分: 在方案5基础上,再深入分析一下`canset的演化流程` |
| --- | --- |
| 说明 | 本表主要用todo4.1-todo4.4把整个canset的演化流程预演分析了下,避免到训练时,再遇到最近的canset训练不顺问题; |
| todo4 | 以前的生成canset比较混乱(在多外都有代码调用生成newCanset),可以先捋一下究竟要统一下在哪里生成比较好; |
|  | **(一) 经查,现代码共有8处生成canset,如下:** |
...
|  | 6. 在pushFrameFinish()中会有实际protoFo和solutionFo类比生成absRCanset (不废弃,但有BUG参考3014a-问题3); |
|  | 7. 在pushFrameFinish()中会将实际protoFo生成为新的canset (自然解,不废弃); |
|  | 8. 在7生成新canset后,会调用canset识别抽象 (canset识别类比全废弃); |
|  | **(二) 具体哪些要保留呢(除下面保留之外的都废弃掉)?如下:** |
|  | todo4.1 自然的解,应直接生成新canset (如3&7) `不用改代码 T`; |
|  | todo4.2a 在推进cansetFo时,"实际"发生的应生成为新的canset `不用改代码,本就如此 T`; |
|  | todo4.2b 然后执行中的解与真实protoFo会进行:"预想与实际"类比,后生成为新的canset (如2&6) `不用改,本就如此 T`; |
|  | todo4.3 迁移后的解,可直接生成新canset (如4&5) `不用改代码 T`; |
|  | todo4.4 为`猜测尝试`生成为canset (这条不是保留,而是为前三个todo补足哪里还需要生成canset) `在todo3已实践 T`; |
| todo5 | 捋一下hCanset现在的生成时机是否ok (在todo4已经捋明白了,todo4-3生成新hCanset,4-2会构建absHCanset) `T`; |
...
| todo7 | "预想与实际"对比出absHCanset后,继承一下预想的spDic `现本就支持,参考27213-5 T`; |
| todo8 | 查下RCanset预想与实际类比,未能执行的BUG (参考3014a-问题3); |
|  | 经调试: 执行"RHCanset预想与实际类比"的条件是状态必须是ActYes或Runing,可能是这个状态不对 (待证实); |
| 结果 | 本节别的ok,只有todo8,需要等cansetFo推进彻底顺当后,才能测到 `转30151-结果`; |

**小结: 30014b的提出方案5: 猜测尝试方案,在3014c和3014d实践改代码,顺带在todo4把整个canset的演化过程整理分析了下;**

| 3014e | 本表分析: cansetA共同抽象匹配后,怎么生成新canset? |
| --- | --- |
| 问题 | 有多帧共同抽象到时,此时怎么遍历所有猜测的canset? |
|  | 示例: 比如有三帧匹配各有3,3,6条共同抽象,难道就要构建54条canset吗? |
| 方案1 | 每一帧都会调用TCRethink,只要每一帧都基于cansetFo重组出共同抽象的条数即可; |
|  | **会构建条数: 即以上会生成3+3+6=12条canset** |
| 方案2 | 将"实际发生"构建为新canset,并且会类比"预想与实际"得到抽象canset (参考上表todo4.2a和b); |
|  | > 说明: 此方案即表示不为共同抽象构建canset,而是让它自己参与"预想与实际"的类比,并得到一个抽象canset; |
|  | **会构建条数: 即以上会生成1条conCanset和1条absCanset** |
| 综合 | 方案2性能更优,还顺便将"预想与实际类比"与"共同抽象构建absCanset"二者完美结合,都完成了,选择方案2,实践如下: |
| todo | `实际生成newCanset`和`预想与实际类比生成absCanset`这两点现在代码本来就是支持的; |
| 结果 | 选定方案2,代码本来就如此不用改 `T`; |

**本节3014a-3014e部分,总结最终做法如下: **
* 本文捋顺了Canset的演化流程 (参考3014d-todo4.1->4.4);
* CansetA的共同抽象匹配=>保留;
* CansetFo的预想与实际类比抽象=>保留;
* newCansetFo触发Canset识别与类比抽象=>废弃;
* cansetFo猜测尝试方案5,其实最终就只体现在cansetA在feedbackTOR()中可以用共同抽象来判断匹配;

***

## n30p15 去皮训练-学会去皮部分5 (捋顺Canset预想与实际类比流程)
`CreateTime 2023.10.29`

本节回测上节3014c&d&e的改动: 废弃了newCansetFo识别类比 & feedbackTOR全面支持cansetAlg的共同抽象即匹配;

| 30151 | 回测 |
| --- | --- |
| 说明 | 上节末改了共同抽象可cansetA匹配后,应该能推进cansetFo了,所以回测下: |
|  | 1. 推进后就会有"预想与实际"类比; |
|  | 2. 正好测下todo8的BUG(参考30143-问题2-复现); |
| 回测1 | 经回测,30143-问题1的rSolution成功率只有5%的问题,未彻底回测这条,不过大致看日志成功率是Ok的; |
| 回测2 | 经回测,30143-问题2的木棒可以反馈上了,只是再往后面继续推进还有问题,日志如下: |
|  | 日志: RCansetA有效: C(A3611) 结果:1 CAtFo:F7362[M1{↑饿-16},A1320(距向,皮果),A3611(向距,棒),A3615(距向,棒),A3616(果),飞↑,A3624(棒),A3625(果),A3630(棒),A3547(吃1)] |
| 结果 | 回测2中,cansetFo推进还是有些问题,这个转下表解决; |

| 30152 | rCanset中有两条木棒帧,导致canset推进不顺问题 `参考30151-回测2-日志`; |
| --- | --- |
| 本质 | 本问题的本质是: **具象canset中有杂质,导致推进困难** |
| 说明 | 从如上表日志可见,第3帧木棒已经反馈成功,但第4帧却没法成功的; |
| 原因 | RCanset生成时,当时可能视觉上可能正好多次看到木棒,但当时都是IN流程,这个很容易跑成这样; |
| 分析 | 但在决策时,这很难复现,因为决策时可能第3帧ok,第4帧就又涉及到OUT流程了,或者很难再在指定时间内又IN一个木棒; |
| 思路 | 所以单纯的IN流程,和IO都在跑的流程,是很难百分百复现的,导致日志中第4帧很难反馈到; |
| 方案1 | 触发rCanset预想与实际再类比,使生成无多余节点的抽象解: 比如[有皮果,棒,无皮果,飞,吃],肯定更容易推进; |
|  | > 可试下: r任务激活rCanset后,再手动跑下`去皮吃到的步骤`,让它再一次生成newCanset,看能不能触发再类比; |
| 方案2 | 触发1s多帧被动视觉,因为在此例中,第二次木棒未能反馈,是因为视觉未触发,并不是因为它无解导致无法推进; |
|  | > 其实被动连续视觉是迟早要做的,只是要多分析下,这个需求现在是否已经迫切度成熟; |
| 决择 | 方案2虽然迟早要搞,但与本表问题本质无关,所以应优先通过方案1来尝试解决 `转下表尝试用训练触发"预想与实际类比" T`; |

| 30153 | 接上表-方案1: 尝试通过训练触发"预想与实际"类比,并得到absCanset |
| --- | --- |
| 训练 | 在FZ884的基础上,然后在动物模式下跑30145-步骤3 (训练带皮学吃),具体步骤如下: |
| 步骤 | 动物模式: `FZ884,路边出生,饿,路上就近扔带皮果,扔棒去皮,手动飞过去,触发吃掉` |
|  | 训练结果: 试了下,跑出日志: `RCanset预想与实际类比未执行,F9092 状态:3(ActNo)` |
| 分析 | 看下此时的日志: 看下是不是有rCanset的执行,然后又有新canset,准备要调用二者类比了 (参考30153-代码段); |
| 方案 | 根据30153-代码段-7线索,可见在ActNo状态时应该也能调用canset预想与实际的类比抽象才对 `T`; |

```c
30153-代码段:
1. `F9092[M1{↑饿-16},A1320(距63,向58,皮果),A9062(向27,距54,棒),A9066(距24,向88,棒),A9067(距10,向90,果),飞↑,A9076(距60,向164,棒),A9077(距0,向91,果),A9082(距62,向166,棒),A3547(吃1)]`
// 解读: 确实有rCanset执行,行为化时序为;

2. `819 [23:40:06:818 TO          TCActYes.m 281] ---//构建行为化帧触发器:0x600001d10780 for:饿 time:0.00`
// 解读: 在第4帧A9076时,取到deltaTime是0;

3. `864 [23:40:08:972 TI          TCActYes.m 306] ---//行为化帧触发理性反省:0x600001d10780 A9066 状态:ActYes`
// 解读: 导致2s后就触发了,触发时它还是ActYes状态;

4. `NSLog(@"tempLogPrefix solution to actNo-------->");`
// 解读: 所以反省显示失败,改为ActNo状态;

5. `---//IP反省触发器执行:0x60000108fb00 F1323 状态:等待反馈`
// 解读: 在IP反省时,rDemand在等待反馈,所以会生成新的rCanset如下条;

6. `R新Canset:F9134[M1{↑饿-16},A1320(距63,向58,皮果),A9105(距50,向30,棒),A9109(距25,向86,棒),A9110(距10,向89,果),飞↑,A9118(向164,距64,棒),A9119(向85,距0,果),A9125(向166,距66,棒),A3547(吃1)] (状态:无反馈 fromPFo:F1323 帧:2)`
// 解读: 在IP反省时,实际发生F9134也生成了新的rCanset;
...
|  | todo5. 回测下todo4.2的改动,不会造成别的影响 `有啥影响现在看不出来,等有了影响再说吧`; |
|  | todo6. 回测下`预想与实际类比`结果会不会还有过度抽象问题 `回测通过,日志见30154-代码段 T`; |
|  | 结果: 加跑30145-步骤3(近中远x上下x各2次)后,RCanset预想与实际类比ok,将结果存为FZ885`RCanset类比日志见代码段2` |
| 测2 | 然后跟着测下hCanset的抽象情况,顺便打日志看下hCanset的强度演化是否正常; |
|  | 问题: `H新Canset`有多次触发,但`hSolution 无计可施`始终无输出; |
|  | 复现: 跑`FZ884,路边出生,饿,路上就近扔带皮果,扔棒去皮,手动飞过去,触发吃掉,睡`,下方近中远各跑一次; |
|  | 答案: 经查,无计可施的目标alg是木棒,木棒本来就是现实世界自行发生的,只有HE行为之后的反馈帧才可能有hCanset解; |
|  | 回顾: 其实在`30143-问题2`遇到过一样的问题,并在30147中给予了解答; |
|  | 结果: 所以此处HCanset对木棒无解是正常的,无解所以无"预想",所以无法触发"预想与实际的类比",所以hCanset演化先不测了; |

```c
30154-代码段:
1. 新proto: F9709[A9677(向5,距254,棒),M1{↑饿-16},A9687(向24,距58,棒),A1320(距63,向58,皮果),A9601(距24,向90,棒),A9693(距10,向96,果),飞↑,A9702(向114,距0,果),A3547(吃1)]
2. 与ass: F9622[M1{↑饿-16},A1320(距63,向58,皮果),A9596(向28,距52,棒),A9601(距24,向90,棒),A9342(距9,向99,果),飞↑,A9609(向163,距59,棒),A9610(距0,向125,果),A9615(距61,向166,棒),A3547(吃1)]
3. 外类比构建时序: F9710[A13(饿16,7),A1320(距63,向58,皮果),A9601(距24,向90,棒),飞↑,A3547(吃1)]->{} from: (protoFo(1):assFo(1))
RCanset预想与实际类比:F9710[A13(饿16,7),A1320(距63,向58,皮果),A9601(距24,向90,棒),飞↑,A3547(吃1)] (curS:F9622 状态:Runing fromPFo:F1323 帧:2)
```

```c
30154-代码段2:
RCanset预想与实际类比:F9097[A13(饿16,7),A1829(距33,向140,皮果),A9096(距9,果),飞↑,A9095(向165,棒),A9094(距0,果),A9093(向168,棒),A3547(吃1)] (curS:F3637 状态:Runing fromPFo:F1832 帧:2)
RCanset预想与实际类比:F9190[A13(饿16,7),A7743(距47,向147,果),A3547(吃1)] (curS:F9098 状态:Runing fromPFo:F1351 帧:2)
RCanset预想与实际类比:F9291[A13(饿16,7),A1112(距63,向276,皮果),A9290(),A9289(距62,棒),A9094(距0,果),A9288(距64,棒),A3547(吃1)] (curS:F7285 状态:Runing fromPFo:F1115 帧:2)
RCanset预想与实际类比:F9336[A13(饿16,7),A1112(距63,向276,皮果),A9290(),A3547(吃1)] (curS:F7285 状态:Runing fromPFo:F1115 帧:2)
RCanset预想与实际类比:F9376[A13(饿16,7),A4836(距67,向141,果),A9375(向163,棒),A3547(吃1)] (curS:F9099 状态:Runing fromPFo:F2531 帧:2)
RCanset预想与实际类比:F9415[A13(饿16,7),A64(向330,距32,皮果),A9290(),A9414(距60,棒),A3547(吃1)] (curS:F9385 状态:Runing fromPFo:F67 帧:2)
RCanset预想与实际类比:F9562[A13(饿16,7),A1829(距33,向140,皮果),A9561(距10,果),飞↑,A9095(向165,棒),A3547(吃1)] (curS:F9372 状态:Runing fromPFo:F1832 帧:2)
RCanset预想与实际类比:F9602[A13(饿16,7),A917(距87,向292,皮果),A9391(距23,向274,棒),飞↓,A9403(向276,距0,果),A3547(吃1)] (curS:F9416 状态:Runing fromPFo:F920 帧:2)
```

**小结: 本节捋了下"预想与实际"类比的流程,目前:**
* RCanset的预想与实际类比回测ok了(参考30154-代码段-3中的absRCanset结果);
* 但HCanset的预想与实际类比还没测好,因为目前不具备测试条件 (目前训练情况不需要HCanset跑那么多,所以没法测) (参考30154-测2-结果);
* 从目前的进度看,后面可以做搬运训练了,等做了搬运训练后,再回来测hCanset的预想与实际类比 (转n30pN-2023.11.09);
...
* 2023.11.04: 在H任务时: 如果iScene没有解,而subScene有解,也要支持推举 (因为hScene是越来越抽象的,在激活抽象hScene时,解大概率是在它的具象subScene里);
* 2023.11.09: TODO-回头等小鸟搬运训练时,回来测下hCanset的演化(尤其是H预想与实际的类比这些) (参考30154-测2-结果);
