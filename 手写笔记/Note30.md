...
明日: 跑起来看下,改了共同抽象可cansetA匹配后,应该更容易推进cansetFo了,推进后就会有"预想与实际"类比,正好测下todo8的BUG(参考30143-问题2-复现);

* 执行30143-问题2-复现后:
  - RCansetA有效:M(A11229) C(A3611) 结果:1 CAtFo:F7362[M1{↑饿-16},A1320(距63,向58,皮果),A3611(向18,距78,棒),A3615(距23,向88,棒),A3616(向93,距9,果),飞↑,A3624(距62,向165,棒),A3625(向108,距0,果),A3630(向168,距64,棒),A3547(吃1)]
* 木棒因为共同抽象有反馈成功;
  - 但再后面的,查下反馈A3611后,再下帧的A3615有没有反馈到,为什么;
