...
|  | 答案: 可以在场景内做alg识别,这样即能匹配到,又不会要求太高 `转方案`; |
...
| 综合 | 问题2提到的:`新canset识别类比`是正途,所以优先根据问题2,来制定方案如下: |
| 方案 | 最好是能支持`场景内alg似层识别`,从而避免`问题2导致很难撞上的问题`,进尔使canset时序识别能成功,类比能成功; |

**小结: 最近几张表中匹配不上的问题困惑,都是源于3014a-问题2中,关于场景不包含帧的匹配问题,且3014a-方案也给出了解决方案,即写canset场景内alg识别算法;**

| 3014b | 写canset场景内alg识别算法 |
| --- | --- |
| 实践规划 | 其实和普通alg识别算法没啥匹配,仅仅是把"识别区域限定在场景内"而已; |
