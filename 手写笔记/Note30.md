...
|  | 方案: 每一帧都会调用TCRethink,只要每一帧都基于cansetFo重组出共同抽象的条数即可,即以上会生成3+3+6=12条canset; |
...
|  | todo4.2 执行中的解与真实protoFo不太一样,可先对(预想与实际)二者类比,后生成为新的canset (如2&6) `不用改代码 T`; |
