...
- [回测防撞训练继续觅食训练](#回测防撞训练继续觅食训练)
  ...
  - [n30p05 迭代TCRefrection反思-多任务综合评价](#n30p05-迭代tcrefrection反思-多任务综合评价)
...
## n30p05 迭代TCRefrection反思-多任务间影响评价
...
| --- | --- |
| 推进1 | **先回顾下现有反馈反思识别的代码:** |
|  | 思路: 查下原来的反思识别的代码,看能不能用子任务综合评分来解决下此问题; |
|  | 现流程: feedbackTOR(反馈判断) -> feedbackRegroup(反馈重组) -> feedbackRecognition(反馈识别) -> feedbackDemand(子任务) `日志如下:` |
|  | ![](assets/702_反思识别有些粗泛.png) |
|  | 说明: 如图,现有反馈反思识别有点简陋,如上图中根据一个疼pFo,识别到另外几个疼pFo,这么嵌套有点原地打转的嫌疑 (HE的循环是禁止原地打转的); |
...
| 重点1 | 需要在输出前,将当前A任务的canset与B任务的cansets进行嫁接,然后看下会对B任务产生什么影响; |
|  | todo1: 将A行为化canset中cutIndex之后,与B任务pFo的cutIndex之前,拼接起来; |
|  | todo2: 然后到B任务的cansets中进行识别,看下识别结果的spEff值; |
|  | todo3: 根据B任务的spEff值,乘以B任务的mv值,得出A任务如果这么做对B任务带来的影响; |
