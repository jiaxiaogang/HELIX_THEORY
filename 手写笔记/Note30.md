...
|  | 缺点: 改动较大,可不仅仅要改概念识别,时序识别也要跟着改,不然是识别不到交层pFos结果的; |
| 方案2 | 在现有pFos的基础上直接取抽象absFo,即可以取得交层,不必改识别算法 `95%`; |
|  | 优点: 此方案简单可行,可以尝试下先: 至少在TI阶段改动很小,但在TO阶段还是免不了改动,具体捋捋需改动如下: |
| 原则 | 关于本表改动,再重点声明下30133的原则: **此处向交层寻求帮助,仅为了提升指导性和迁移性,别的不变;** |
| todo1 | 生成新的rCanset时,也要取具象层,然后将新rCanset扩给交层一份; |
|  | 调试1. 先调试下跑起来日志,看下现在交层是不是已经迁移上了rCanset,如果有当然最好,90%的改动省得动; |
|  | 调试2. 如果发现怎么训练在交层也不会有rCanset,或很难有,再写todo1; |
