...
| 步骤3 | **学H去皮: 动物模式,(`饿,路中扔有皮果,扔木棒`) x 上下各2次** |
...
| 步骤4 | **学能否压到: 动物模式,(`路边随机出生,饿,随机位置扔有皮果,扔木棒` x 200轮) x 路上下边各1次;** |
|  | 示图: 要分析hCanset的自由竞争演化,可以借助示图来分析如下: |
|  | ![](assets/709_hCanset演化分析.png) |
|  | 训练目标: 学什么时候能压到,什么时候压不到 (因为只有知道什么时候压不到,才能有想被压到的`搬运动机`); |
|  | TIPS: 其实只需要和rScene只训练有mv指向的pFo一样,此处也**只需要训练能够压到的hCanset及其SP稳定率**; |
